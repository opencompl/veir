...
// An access decodes to the object its address falls into and must stay
// inside it. The second 8-byte alloca starts 16 bytes after the first, so an
// 8-byte store at offset 12 of the first would reach into the second and is UB.
