// RUN: veir-interpret %s | filecheck %s

// Initialize the 16 bytes of a stack slot so that each byte contains its offset,
// using two doubleword stores (little-endian):
//   sd @+0 = 0x0706050403020100  -> bytes 0..7  = 00 01 02 03 04 05 06 07
//   sd @+8 = 0x0F0E0D0C0B0A0908  -> bytes 8..15 = 08 09 0A 0B 0C 0D 0E 0F
// Then a misaligned doubleword load from offset 3 reads bytes 3..10:
//   03 04 05 06 07 08 09 0A, which little-endian is 0x0a09080706050403.

"builtin.module"() ({
  "func.func"() <{sym_name = "main", function_type = () -> !riscv.reg}> ({
    %a0 = "riscv_stack.alloca"() <{ "size" = 16 : i64, "alignment" = 8 : i64 }> : () -> !riscv.reg
    %v0 = "riscv.li"() <{ "value" = 506097522914230528 : i64 }> : () -> !riscv.reg
    "riscv.sd"(%v0, %a0) <{ "value" = 0 : i64 }> : (!riscv.reg, !riscv.reg) -> ()

    %a8 = "riscv.addi"(%a0) <{ value = 8 : i64 }> : (!riscv.reg) -> !riscv.reg
    %v8 = "riscv.li"() <{ "value" = 1084818905618843912 : i64 }> : () -> !riscv.reg
    "riscv.sd"(%v8, %a8) <{ "value" = 0 : i64 }> : (!riscv.reg, !riscv.reg) -> ()

    %y = "riscv.ld"(%a0) <{ "value" = 3 : i64 }> : (!riscv.reg) -> !riscv.reg
    "func.return"(%y) : (!riscv.reg) -> ()
  }) : () -> ()
}) : () -> ()

// CHECK: Program output: #[0x0a09080706050403#64]
