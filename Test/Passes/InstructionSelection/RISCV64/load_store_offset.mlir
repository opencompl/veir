...
"builtin.module"() ({
    ...
    // Folding uses allocation stride, not store size: i24 has store size 3 but
    // RV64 allocation stride 4, so index 2 gives byte offset 8.
    "func.func"()  <{function_type = (!llvm.ptr) -> (), sym_name = "fold_i24_stride"}> ({
    ^bb0(%p: !llvm.ptr):
        %i = "llvm.mlir.constant"() <{value = 2 : i64}> : () -> i64
        %g = "llvm.getelementptr"(%p, %i) <{elem_type = i24, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
        %v = "llvm.load"(%g) : (!llvm.ptr) -> i32
        // CHECK:      {{.*}} = "builtin.unrealized_conversion_cast"({{.*}}) : (!llvm.ptr) -> !riscv.reg
        // CHECK-NEXT: {{.*}} = "riscv.lw"({{.*}}) <{"value" = 8 : i64}> : (!riscv.reg) -> !riscv.reg
        // CHECK-NEXT: {{.*}} = "builtin.unrealized_conversion_cast"({{.*}}) : (!riscv.reg) -> i32
        "test.test"(%v) : (i32) -> ()
        "func.return"() : () -> ()
    }) : () -> ()
