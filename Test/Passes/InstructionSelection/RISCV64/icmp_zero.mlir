...
// Peephole: comparing against a constant 0 lowers `eq`/`ne` directly on the
// non-zero operand, with no `riscv.xor`.
...
"builtin.module"() ({
    ...
    // eq, zero on the left: 0 == a  (still lowers directly on a)
    "func.func"() <{function_type = (i64) -> (i1)}> ({
    ^bb0(%a: i64):
        %z = "llvm.mlir.constant"() <{value = 0 : i64}> : () -> i64
        %r = "llvm.icmp"(%z, %a) <{predicate = 0 : i64}> : (i64, i64) -> i1
        "func.return"(%r) : (i1) -> ()
        // CHECK-LABEL: "func.func"
        // CHECK: "riscv.sltiu"(%{{.*}}) <{"value" = 1 : i64}> : (!riscv.reg) -> !riscv.reg
        // CHECK-NOT: "riscv.xor"
