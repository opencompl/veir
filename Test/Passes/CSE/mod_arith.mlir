...
"builtin.module"() ({
  ...
  // A dominating mod_arith definition is reused across basic blocks.
  "func.func"() <{function_type = (!mod_arith.int<7 : i32>, !mod_arith.int<7 : i32>) -> (), sym_name = "cross_block"}> ({
  ^entry(%a : !mod_arith.int<7 : i32>, %b : !mod_arith.int<7 : i32>):
    %m0 = "mod_arith.mul"(%a, %b) : (!mod_arith.int<7 : i32>, !mod_arith.int<7 : i32>) -> !mod_arith.int<7 : i32>
    "test.test"(%m0) : (!mod_arith.int<7 : i32>) -> ()
    "cf.br"() [^next] : () -> ()
  ^next:
    %m1 = "mod_arith.mul"(%b, %a) : (!mod_arith.int<7 : i32>, !mod_arith.int<7 : i32>) -> !mod_arith.int<7 : i32>
    "test.test"(%m1) : (!mod_arith.int<7 : i32>) -> ()
    "func.return"() : () -> ()

    // CHECK-LABEL: "sym_name" = "cross_block"
    // CHECK:      %[[M:.*]] = "mod_arith.mul"
    // CHECK-NEXT: "test.test"(%[[M]])
    // CHECK:      ^{{[0-9]+}}():
    // CHECK-NEXT: "test.test"(%[[M]])
