...
"builtin.module"() ({
  ...
  ^bb0(%x: i8, %y: i8, %p: i1, %q: i1):
    ...
    // The one bit pattern also matches at i1, so (p - q) - 1 becomes ~q + p.
    // (1 - 0) - 1 is zero, but ~0 + 1 overflows both ways.
...
// CHECK-NEXT: %[[X0:.*]] = "llvm.xor"(%[[Q]], %[[O0]]) : (i1, i1) -> i1
// CHECK-NEXT: %[[N0:.*]] = "llvm.add"(%[[X0]], %[[P]]) : (i1, i1) -> i1
// CHECK-NEXT: %[[O1:.*]] = "llvm.mlir.constant"() <{"value" = -1 : i1}>
// CHECK-NEXT: %[[X1:.*]] = "llvm.xor"(%[[Q]], %[[O1]]) : (i1, i1) -> i1
// CHECK-NEXT: %[[N1:.*]] = "llvm.add"(%[[X1]], %[[P]]) : (i1, i1) -> i1
// CHECK-NEXT: %[[O2:.*]] = "llvm.mlir.constant"() <{"value" = -1 : i1}>
// CHECK-NEXT: %[[X2:.*]] = "llvm.xor"(%[[Q]], %[[O2]]) : (i1, i1) -> i1
// CHECK-NEXT: %[[N2:.*]] = "llvm.add"(%[[X2]], %[[P]]) : (i1, i1) -> i1
