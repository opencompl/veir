...
An MLIR operation.
-/
structure Operation (dT : Type) [HasProperties dT] where
...
variable { dT } [HasProperties dT]

namespace Operation

theorem default_operands_eq :
    (default : Operation dT).operands = #[] := by
  rfl

theorem default_regions_eq :
    (default : Operation dT).regions = #[] := by
  rfl

theorem default_blockOperands_eq :
    (default : Operation dT).blockOperands = #[] := by
  rfl

theorem default_results_eq :
    (default : Operation dT).results = #[] := by
  rfl
...
-/
structure IRContext (dT : Type) [HasProperties dT] where
  operations : HashMap OperationPtr (Operation dT)
...
@[expose]
def Operation.empty (opType : dT) (prop : HasProperties.propertiesOf opType) : (Operation dT) :=
  { results := #[]
    prev := none
    next := none
    parent := none
...
@[local grind]
def InBounds (op : OperationPtr) (ctx : IRContext dT) : Prop :=
  op ∈ ctx.operations

def get (ptr : OperationPtr) (ctx : IRContext dT) (inBounds : ptr.InBounds ctx := by grind) : (Operation dT) :=
  ctx.operations[ptr]'(by unfold InBounds at inBounds; grind)

def get! (ptr : OperationPtr) (ctx : IRContext dT) : (Operation dT) :=
  ctx.operations[ptr]!
@[grind _=_]
...
def getOpType (op : OperationPtr) (ctx : IRContext dT) (inBounds : op.InBounds ctx) : dT :=
  (op.get ctx (by grind)).opType

def getNumOperands (op : OperationPtr) (ctx : IRContext dT) (inBounds : op.InBounds ctx := by grind) : Nat :=
  (op.get ctx (by grind)).operands.size

def getNumOperands! (op : OperationPtr) (ctx : IRContext dT) : Nat :=
...
def getOperand (op : OperationPtr) (ctx : IRContext dT) (index : Nat)
...
def getOperand! (op : OperationPtr) (ctx : IRContext dT) (index : Nat) : ValuePtr :=
...
def getNumSuccessors (op : OperationPtr) (ctx : IRContext dT) (inBounds : op.InBounds ctx := by grind) : Nat :=
  (op.get ctx (by grind)).blockOperands.size

def getNumSuccessors! (op : OperationPtr) (ctx : IRContext dT) : Nat :=
...
def getSuccessor (op : OperationPtr) (ctx : IRContext dT) (index : Nat)
...
def getSuccessor! (op : OperationPtr) (ctx : IRContext dT) (index : Nat) : BlockPtr :=
...
def getNumResults (op : OperationPtr) (ctx : IRContext dT) (inBounds : op.InBounds ctx := by grind) : Nat :=
  (op.get ctx (by grind)).results.size

def getNumResults! (op : OperationPtr) (ctx : IRContext dT) : Nat :=
...
def getNumRegions (op : OperationPtr) (ctx : IRContext dT)
    (inBounds : op.InBounds ctx := by grind) : Nat :=
  (op.get ctx (by grind)).regions.size

def getNumRegions! (op : OperationPtr) (ctx : IRContext dT) : Nat :=
...
def getRegion (op : OperationPtr) (ctx : IRContext dT) (index : Nat)
...
@[grind funCC]
def getRegion! (op : OperationPtr) (ctx : IRContext dT) (index : Nat) : RegionPtr :=
...
def set (ptr : OperationPtr) (ctx : IRContext dT) (newOp : Operation dT) : IRContext dT :=
  {ctx with operations := ctx.operations.insert ptr newOp}

def setNextOp (op : OperationPtr) (ctx : IRContext dT) (newNext : Option OperationPtr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx
  op.set ctx { oldOp with next := newNext}

def setNextOp! (op : OperationPtr) (ctx : IRContext dT) (newNext : Option OperationPtr) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with next := newNext}
...
def setPrevOp (op : OperationPtr) (ctx : IRContext dT) (newPrev : Option OperationPtr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with prev := newPrev}

def setPrevOp! (op : OperationPtr) (ctx : IRContext dT) (newPrev : Option OperationPtr) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with prev := newPrev}
...
def setParent (op : OperationPtr) (ctx : IRContext dT) (newParent : Option BlockPtr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with parent := newParent}

def setParent! (op : OperationPtr) (ctx : IRContext dT) (newParent : Option BlockPtr) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with parent := newParent}
...
def setRegions (op : OperationPtr) (ctx : IRContext dT) (newRegions : Array RegionPtr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with regions := newRegions}

def setRegions! (op : OperationPtr) (ctx : IRContext dT) (newRegions : Array RegionPtr) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with regions := newRegions}
...
def pushRegion (op : OperationPtr) (ctx : IRContext dT) (reg : RegionPtr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
...
def setResults (op : OperationPtr) (ctx : IRContext dT) (newResults : Array OpResult)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with results := newResults}

def setResults! (op : OperationPtr) (ctx : IRContext dT) (newResults : Array OpResult) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with results := newResults}
...
def pushResult (op : OperationPtr) (ctx : IRContext dT) (resultS : OpResult)
      (hop : op.InBounds ctx := by grind) :=
    op.setResults ctx ((op.get ctx).results.push resultS)

def pushResult! (op : OperationPtr) (ctx : IRContext dT) (resultS : OpResult) : IRContext dT :=
  op.setResults! ctx ((op.get! ctx).results.push resultS)
...
def setBlockOperands (op : OperationPtr) (ctx : IRContext dT) (newOperands : Array BlockOperand)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
...
def pushBlockOperand (op : OperationPtr) (ctx : IRContext dT) (operands : BlockOperand)
      (hop : op.InBounds ctx := by grind) :=
...
def setOperands (op : OperationPtr) (ctx : IRContext dT) (newOperands : Array OpOperand)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with operands := newOperands}

def setOperands! (op : OperationPtr) (ctx : IRContext dT) (newOperands : Array OpOperand) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with operands := newOperands}
...
def pushOperand (op : OperationPtr) (ctx : IRContext dT) (operandS : OpOperand)
      (hop : op.InBounds ctx := by grind) :=
    op.setOperands ctx ((op.get ctx).operands.push operandS)

def pushOperand! (op : OperationPtr) (ctx : IRContext dT) (operands : OpOperand) : IRContext dT :=
  op.setOperands! ctx ((op.get! ctx).operands.push operands)
...
def setAttributes (op : OperationPtr) (ctx : IRContext dT) (newAttrs : DictionaryAttr)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
  let oldOp := op.get ctx
  op.set ctx { oldOp with attrs := newAttrs}

def setAttributes! (op : OperationPtr) (ctx : IRContext dT) (newAttrs : DictionaryAttr) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with attrs := newAttrs}
...
@[inline]
def getProperties (op : OperationPtr) (ctx : IRContext dT) (opCode : dT)
    (inBounds : op.InBounds ctx := by grind)
    (hprop : (op.get ctx inBounds).opType = opCode := by grind) : HasProperties.propertiesOf opCode :=
  hprop ▸ (op.get ctx (by grind)).properties

@[inline]
def getProperties! (op : OperationPtr) (ctx : IRContext dT) (opCode : dT) : HasProperties.propertiesOf opCode :=
...
def setProperties {opCode : dT } (op : OperationPtr) (ctx : IRContext dT)
    (newProperties : HasProperties.propertiesOf opCode)
    (inBounds : op.InBounds ctx := by grind)
    (hprop : (op.get ctx inBounds).opType = opCode := by grind) : IRContext dT :=
  let oldOp := op.get ctx (by grind)
  op.set ctx { oldOp with properties := hprop ▸ newProperties }

def setProperties! {opCode : dT} (op : OperationPtr) (ctx : IRContext dT)
  (newProperties : HasProperties.propertiesOf opCode)
  (hprop : (op.get! ctx).opType = opCode := by grind) : IRContext dT :=
  let oldOp := op.get! ctx
  op.set ctx { oldOp with properties := hprop ▸ newProperties }

@[grind _=_]
theorem setProperties!_eq_setProperties {op : OperationPtr}
    (newProperties : HasProperties.propertiesOf opCode) (inBounds : op.InBounds ctx)
...
@[grind]
def nextOperand (op : OperationPtr) (ctx : IRContext dT)
    (inBounds : op.InBounds ctx := by grind) : OpOperandPtr :=
  .mk op (op.getNumOperands ctx (by grind))

@[grind]
def nextOperand! (op : OperationPtr) (ctx : IRContext dT) : OpOperandPtr :=
  .mk op (op.getNumOperands! ctx)

@[grind]
def nextBlockOperand (op : OperationPtr) (ctx : IRContext dT)
    (inBounds : op.InBounds ctx := by grind) : BlockOperandPtr :=
  .mk op (op.getNumSuccessors ctx (by grind))

@[grind]
def nextBlockOperand! (op : OperationPtr) (ctx : IRContext dT) : BlockOperandPtr :=
  .mk op (op.getNumSuccessors! ctx)

@[grind]
def nextResult (op : OperationPtr) (ctx : IRContext dT)
    (inBounds : op.InBounds ctx := by grind) : OpResultPtr :=
  .mk op (op.getNumResults ctx (by grind))

@[grind]
def nextResult! (op : OperationPtr) (ctx : IRContext dT) : OpResultPtr :=
  .mk op (op.getNumResults! ctx)

def allocEmpty (ctx : IRContext dT) (opType : dT) (properties : HasProperties.propertiesOf opType) :
    Option (IRContext dT × OperationPtr) :=
...
def dealloc (op : OperationPtr) (ctx : IRContext dT)
    (inBounds : op.InBounds ctx := by grind) : IRContext dT :=
...
@[local grind]
def InBounds (operand : OpOperandPtr) (ctx : IRContext dT) : Prop :=
...
def set (operand : OpOperandPtr) (ctx : IRContext dT) (newOperand : OpOperand)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
...
def set! (operand : OpOperandPtr) (ctx : IRContext dT) (newOperand : OpOperand) : IRContext dT :=
  let op := operand.op.get! ctx
  { ctx with
    operations := ctx.operations.insert operand.op
      { op with
...
def setNextUse (operand : OpOperandPtr) (ctx : IRContext dT) (newNextUse : Option OpOperandPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with nextUse := newNextUse }

def setNextUse! (operand : OpOperandPtr) (ctx : IRContext dT) (newNextUse : Option OpOperandPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with nextUse := newNextUse }
...
def setBack (operand : OpOperandPtr) (ctx : IRContext dT) (newBack : OpOperandPtrPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with back := newBack }

def setBack! (operand : OpOperandPtr) (ctx : IRContext dT) (newBack : OpOperandPtrPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with back := newBack }
...
def setOwner (operand : OpOperandPtr) (ctx : IRContext dT) (newOwner : OperationPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with owner := newOwner }

def setOwner! (operand : OpOperandPtr) (ctx : IRContext dT) (newOwner : OperationPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with owner := newOwner }
...
def setValue (operand : OpOperandPtr) (ctx : IRContext dT) (newValue : ValuePtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with value := newValue }

def setValue! (operand : OpOperandPtr) (ctx : IRContext dT) (newValue : ValuePtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with value := newValue }
...
@[local grind]
def InBounds (operand : BlockOperandPtr) (ctx : IRContext dT) : Prop :=
...
@[grind .]
theorem inBounds_of_OperationPtr_inBounds {operand : BlockOperandPtr} {ctx : IRContext dT} :
    operand.op.InBounds ctx →
    operand.index < operand.op.getNumSuccessors! ctx →
    operand.InBounds ctx :=
  by grind [inBounds_def]

def get (operand : BlockOperandPtr) (ctx : IRContext dT) (operandIn : operand.InBounds ctx := by grind) : BlockOperand :=
  (operand.op.get ctx (by grind [InBounds])).blockOperands[operand.index]'(by grind [InBounds])
def get! (operand : BlockOperandPtr) (ctx : IRContext dT) : BlockOperand :=
...
def set (operand : BlockOperandPtr) (ctx : IRContext dT) (newOperand : BlockOperand) (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let op := operand.op.get ctx
  { ctx with
    operations := ctx.operations.insert operand.op
      { op with
...
def set! (operand : BlockOperandPtr) (ctx : IRContext dT) (newOperand : BlockOperand) : IRContext dT :=
  let op := operand.op.get! ctx
  { ctx with
    operations := ctx.operations.insert operand.op
      { op with
...
def setNextUse (operand : BlockOperandPtr) (ctx : IRContext dT) (newNextUse : Option BlockOperandPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with nextUse := newNextUse }
...
def setBack (operand : BlockOperandPtr) (ctx : IRContext dT) (newBack : BlockOperandPtrPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with back := newBack }

def setBack! (operand : BlockOperandPtr) (ctx : IRContext dT) (newBack : BlockOperandPtrPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with back := newBack }
...
def setOwner (operand : BlockOperandPtr) (ctx : IRContext dT) (newOwner : OperationPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with owner := newOwner }

def setOwner! (operand : BlockOperandPtr) (ctx : IRContext dT) (newOwner : OperationPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with owner := newOwner }
...
def setValue (operand : BlockOperandPtr) (ctx : IRContext dT) (newValue : BlockPtr)
    (operandIn : operand.InBounds ctx := by grind) : IRContext dT :=
  let oldOperand := operand.get ctx
  operand.set ctx { oldOperand with value := newValue }

def setValue! (operand : BlockOperandPtr) (ctx : IRContext dT) (newValue : BlockPtr) : IRContext dT :=
  let oldOperand := operand.get! ctx
  operand.set! ctx { oldOperand with value := newValue }
...
@[local grind]
def InBounds (result : OpResultPtr) (ctx : IRContext dT) : Prop :=
...
@[grind .]
theorem inBounds_OperationPtr_getNumResults! (result : OpResultPtr) (ctx : IRContext dT) (h : result.InBounds ctx) :
...
def get (result : OpResultPtr) (ctx : IRContext dT) (resultIn : result.InBounds ctx := by grind) : OpResult :=
  (result.op.get ctx (by grind [InBounds])).results[result.index]'(by grind [InBounds])

def get! (result : OpResultPtr) (ctx : IRContext dT) : OpResult :=
...
def set (result : OpResultPtr) (ctx : IRContext dT) (newresult : OpResult) (resultIn : result.InBounds ctx := by grind) : IRContext dT :=
...
def set! (result : OpResultPtr) (ctx : IRContext dT) (newresult : OpResult) : IRContext dT :=
  let op := result.op.get! ctx
  { ctx with
    operations := ctx.operations.insert result.op
...
def setType (result : OpResultPtr) (ctx : IRContext dT) (newType : TypeAttr)
    (resultIn : result.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := result.get ctx
  result.set ctx { oldResult with type := newType }

def setType! (result : OpResultPtr) (ctx : IRContext dT) (newType : TypeAttr) : IRContext dT :=
  let oldResult := result.get! ctx
  result.set! ctx { oldResult with type := newType }
...
def setFirstUse (result : OpResultPtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr)
    (resultIn : result.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := result.get ctx
  result.set ctx { oldResult with firstUse := newFirstUse }

def setFirstUse! (result : OpResultPtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr) : IRContext dT :=
  let oldResult := result.get! ctx
  result.set! ctx { oldResult with firstUse := newFirstUse }
...
def setOwner (result : OpResultPtr) (ctx : IRContext dT) (newOwner : OperationPtr)
    (resultIn : result.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := result.get ctx
  result.set ctx { oldResult with owner := newOwner }

def setOwner! (result : OpResultPtr) (ctx : IRContext dT) (newOwner : OperationPtr) : IRContext dT :=
  let oldResult := result.get! ctx
  result.set! ctx { oldResult with owner := newOwner }
...
def InBounds (block : BlockPtr) (ctx : IRContext dT) : Prop :=
  block ∈ ctx.blocks

def get (ptr : BlockPtr) (ctx : IRContext dT) (inBounds : ptr.InBounds ctx := by grind) : Block :=
  ctx.blocks[ptr]'(by unfold InBounds at inBounds; grind)

def get! (ptr : BlockPtr) (ctx : IRContext dT) : Block := ctx.blocks[ptr]!
...
def set (ptr : BlockPtr) (ctx : IRContext dT) (newBlock : Block) : IRContext dT :=
  {ctx with blocks := ctx.blocks.insert ptr newBlock}

def setParent (block : BlockPtr) (ctx : IRContext dT) (newParent : Option RegionPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with parent := newParent}

def setParent! (block : BlockPtr) (ctx : IRContext dT) (newParent : Option RegionPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with parent := newParent}
...
def setFirstUse (block : BlockPtr) (ctx : IRContext dT) (newFirstUse : Option BlockOperandPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with firstUse := newFirstUse}

def setFirstUse! (block : BlockPtr) (ctx : IRContext dT) (newFirstUse : Option BlockOperandPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with firstUse := newFirstUse}
...
def setFirstOp (block : BlockPtr) (ctx : IRContext dT) (newFirstOp : Option OperationPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with firstOp := newFirstOp}

def setFirstOp! (block : BlockPtr) (ctx : IRContext dT) (newFirstOp : Option OperationPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with firstOp := newFirstOp}
...
def setLastOp (block : BlockPtr) (ctx : IRContext dT) (newLastOp : Option OperationPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with lastOp := newLastOp}

def setLastOp! (block : BlockPtr) (ctx : IRContext dT) (newLastOp : Option OperationPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with lastOp := newLastOp}
...
def setNextBlock (block : BlockPtr) (ctx : IRContext dT) (newNext : Option BlockPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with next := newNext}

def setNextBlock! (block : BlockPtr) (ctx : IRContext dT) (newNext : Option BlockPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with next := newNext}
...
def setPrevBlock (block : BlockPtr) (ctx : IRContext dT) (newPrev : Option BlockPtr)
    (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx
  block.set ctx { oldBlock with prev := newPrev}

def setPrevBlock! (block : BlockPtr) (ctx : IRContext dT) (newPrev : Option BlockPtr) : IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx {oldBlock with prev := newPrev}
...
def allocEmpty (ctx : IRContext dT) : Option (IRContext dT × BlockPtr) :=
  let newBlockPtr : BlockPtr := ⟨ctx.nextID⟩
  let ctx : IRContext dT := { ctx with nextID := ctx.nextID + 1}
...
def getNumArguments (block : BlockPtr) (ctx : IRContext dT) (inBounds : block.InBounds ctx := by grind) : Nat :=
  (block.get ctx (by grind)).arguments.size

def getNumArguments! (block : BlockPtr) (ctx : IRContext dT) : Nat :=
...
@[grind]
def nextArgument (block : BlockPtr) (ctx : IRContext dT)
    (inBounds: block.InBounds ctx := by grind) : BlockArgumentPtr :=
  getArgument block (block.getNumArguments ctx (by grind))

@[grind]
def nextArgument! (block : BlockPtr) (ctx : IRContext dT) : BlockArgumentPtr :=
  getArgument block (block.getNumArguments! ctx)

def setArguments (block : BlockPtr) (ctx : IRContext dT)
    (newArguments : Array BlockArgument) (inBounds : block.InBounds ctx := by grind) : IRContext dT :=
  let oldBlock := block.get ctx (by grind)
  block.set ctx { oldBlock with arguments := newArguments }

def setArguments! (block : BlockPtr) (ctx : IRContext dT) (newArguments : Array BlockArgument) :
    IRContext dT :=
  let oldBlock := block.get! ctx
  block.set ctx { oldBlock with arguments := newArguments }
...
def pushArgument (block : BlockPtr) (ctx : IRContext dT) (result : BlockArgument)
      (inBounds : block.InBounds ctx := by grind) :=
    block.setArguments ctx ((block.get ctx).arguments.push result)

def pushArgument! (block : BlockPtr) (ctx : IRContext dT) (result : BlockArgument) : IRContext dT :=
  block.setArguments! ctx ((block.get! ctx).arguments.push result)
...
@[local grind]
def InBounds (arg : BlockArgumentPtr) (ctx : IRContext dT) : Prop :=
...
@[grind .]
theorem InBounds_iff (arg : BlockArgumentPtr) (ctx : IRContext dT) :
    arg.block.InBounds ctx →
    arg.index < arg.block.getNumArguments! ctx →
    arg.InBounds ctx :=
  by grind [inBounds_def]

def get (arg : BlockArgumentPtr) (ctx : IRContext dT) (argIn : arg.InBounds ctx := by grind) : BlockArgument :=
  (arg.block.get ctx (by grind [InBounds])).arguments[arg.index]'(by grind [InBounds])

def get! (arg : BlockArgumentPtr) (ctx : IRContext dT) : BlockArgument :=
...
def set (arg : BlockArgumentPtr) (ctx : IRContext dT) (newresult : BlockArgument) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
...
def set! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newresult : BlockArgument) : IRContext dT :=
  let block := arg.block.get! ctx
  { ctx with
    blocks := ctx.blocks.insert arg.block
...
def setType (arg : BlockArgumentPtr) (ctx : IRContext dT) (newType : TypeAttr) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := arg.get ctx
  arg.set ctx { oldResult with type := newType }

def setType! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newType : TypeAttr) : IRContext dT :=
  let oldResult := arg.get! ctx
  arg.set! ctx { oldResult with type := newType }
...
def setFirstUse (arg : BlockArgumentPtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := arg.get ctx
  arg.set ctx { oldResult with firstUse := newFirstUse }

def setFirstUse! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr) :
    IRContext dT :=
  let oldResult := arg.get! ctx
  arg.set! ctx {oldResult with firstUse := newFirstUse}
...
def setIndex (arg : BlockArgumentPtr) (ctx : IRContext dT) (newIndex : Nat) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := arg.get ctx
  arg.set ctx { oldResult with index := newIndex }

def setIndex! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newIndex : Nat) : IRContext dT :=
  let oldResult := arg.get! ctx
  arg.set! ctx {oldResult with index := newIndex}
...
def setLoc (arg : BlockArgumentPtr) (ctx : IRContext dT) (newLoc : Location) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := arg.get ctx
  arg.set ctx { oldResult with loc := newLoc }

def setLoc! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newLoc : Location) : IRContext dT :=
  let oldResult := arg.get! ctx
  arg.set! ctx {oldResult with loc := newLoc}
...
def setOwner (arg : BlockArgumentPtr) (ctx : IRContext dT) (newOwner : BlockPtr) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  let oldResult := arg.get ctx
  arg.set ctx { oldResult with owner := newOwner }

def setOwner! (arg : BlockArgumentPtr) (ctx : IRContext dT) (newOwner : BlockPtr) : IRContext dT :=
  let oldResult := arg.get! ctx
  arg.set! ctx {oldResult with owner := newOwner}
...
inductive InBounds : ValuePtr → IRContext dT → Prop
...
@[simp, grind=]
theorem inBounds_opResult (ptr : OpResultPtr) (ctx : IRContext dT) :
    (opResult ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind [InBounds]

@[simp, grind=]
theorem inBounds_blockArg (ptr : BlockArgumentPtr) (ctx : IRContext dT) :
    (blockArgument ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind [InBounds]

def getType (arg : ValuePtr) (ctx : IRContext dT) (argIn : arg.InBounds ctx := by grind) : TypeAttr :=
  match arg with
  | opResult ptr => (ptr.get ctx (by grind)).type
  | blockArgument ptr => (ptr.get ctx (by grind)).type

def getType! (arg : ValuePtr) (ctx : IRContext dT) : TypeAttr :=
  match arg with
...
def getFirstUse (arg : ValuePtr) (ctx : IRContext dT) (argIn : arg.InBounds ctx := by grind) : Option OpOperandPtr :=
  match arg with
  | opResult ptr => (ptr.get ctx).firstUse
  | blockArgument ptr => (ptr.get ctx).firstUse

def getFirstUse! (arg : ValuePtr) (ctx : IRContext dT) : Option OpOperandPtr :=
  match arg with
  | opResult ptr => (ptr.get! ctx).firstUse
  | blockArgument ptr => (ptr.get! ctx).firstUse
...
@[simp, grind =]
theorem getFirstUse_opResult_eq {res : OpResultPtr} {ctx : IRContext dT} {h : res.InBounds ctx} :
    (opResult res).getFirstUse ctx = (res.get ctx).firstUse := by
  grind [getFirstUse]

@[simp, grind =]
theorem getFirstUse_blockArgument_eq {ba : BlockArgumentPtr} {ctx : IRContext dT} {h : ba.InBounds ctx} :
    (blockArgument ba).getFirstUse ctx = (ba.get ctx).firstUse := by
  grind [getFirstUse]

@[simp, grind =]
theorem getFirstUse!_opResult_eq {res : OpResultPtr} {ctx : IRContext dT} :
    (opResult res).getFirstUse! ctx = (res.get! ctx).firstUse := by
  grind [getFirstUse!]

@[simp, grind =]
theorem getFirstUse!_blockArgument_eq {ba : BlockArgumentPtr} {ctx : IRContext dT} :
    (blockArgument ba).getFirstUse! ctx = (ba.get! ctx).firstUse := by
  grind [getFirstUse!]
...
-/
def hasUses (value : ValuePtr) (ctx : IRContext dT) (valueIn : value.InBounds ctx := by grind) : Bool :=
...
Returns true if the value has any uses.
-/
def hasUses! (value : ValuePtr) (ctx : IRContext dT) : Bool :=
...
def setType (arg : ValuePtr) (ctx : IRContext dT) (newType : TypeAttr) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  match arg with
  | opResult ptr => ptr.setType ctx newType
  | blockArgument ptr => ptr.setType ctx newType

def setType! (arg : ValuePtr) (ctx : IRContext dT) (newType : TypeAttr) : IRContext dT :=
  match arg with
  | opResult ptr => ptr.setType! ctx newType
  | blockArgument ptr => ptr.setType! ctx newType
...
def setFirstUse (arg : ValuePtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr) (argIn : arg.InBounds ctx := by grind) : IRContext dT :=
  match arg with
  | opResult ptr => ptr.setFirstUse ctx newFirstUse
  | blockArgument ptr => ptr.setFirstUse ctx newFirstUse

def setFirstUse! (arg : ValuePtr) (ctx : IRContext dT) (newFirstUse : Option OpOperandPtr) : IRContext dT :=
  match arg with
  | opResult ptr => ptr.setFirstUse! ctx newFirstUse
  | blockArgument ptr => ptr.setFirstUse! ctx newFirstUse
...
@[simp, grind =]
theorem setFirstUse_OpResultPtr (ptr : OpResultPtr) (ctx : IRContext dT)
...
@[simp, grind =]
theorem setFirstUse_BlockArgumentPtr (ptr : BlockArgumentPtr) (ctx : IRContext dT)
...
@[simp, grind =]
theorem setType_OpResultPtr (ptr : OpResultPtr) (ctx : IRContext dT)
    (ptrIn : (opResult ptr).InBounds ctx) (newType : TypeAttr) :
    (opResult ptr).setType ctx newType ptrIn = ptr.setType ctx newType := by
  unfold setType; rfl

@[simp, grind =]
theorem setType_BlockArgumentPtr (ptr : BlockArgumentPtr) (ctx : IRContext dT)
...
inductive InBounds (ctx : IRContext dT) : OpOperandPtrPtr → Prop
...
@[simp, grind=]
theorem inBounds_operandNextUse (ptr : OpOperandPtr) (ctx : IRContext dT) :
    (operandNextUse ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind [InBounds]

@[simp, grind=]
theorem inBounds_valueFirstUse (ptr : ValuePtr) (ctx : IRContext dT) :
    (valueFirstUse ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind [InBounds]

def get (ptrPtr : OpOperandPtrPtr) (ctx : IRContext dT) (ptrPtrIn : ptrPtr.InBounds ctx := by grind) : Option OpOperandPtr :=
  match ptrPtr with
  | operandNextUse ptr => (ptr.get ctx).nextUse
  | valueFirstUse val => val.getFirstUse ctx (by grind)

def get! (ptrPtr : OpOperandPtrPtr) (ctx : IRContext dT) : Option OpOperandPtr :=
  match ptrPtr with
  | operandNextUse ptr => (ptr.get! ctx).nextUse
...
@[simp, grind =]
theorem get_operandNextUse_eq {ptr : OpOperandPtr} {ctx : IRContext dT} {ptrIn : ptr.InBounds ctx} :
    (operandNextUse ptr).get ctx (by grind) = (ptr.get ctx).nextUse := by
  grind [get]

@[simp, grind =]
theorem get!_operandNextUse_eq {ptr : OpOperandPtr} {ctx : IRContext dT} :
    (operandNextUse ptr).get! ctx = (ptr.get! ctx).nextUse := by
  grind [get!]

@[simp, grind =]
theorem get_valueFirstUse_eq {ptr : ValuePtr} {ctx : IRContext dT} {ptrIn : ptr.InBounds ctx} :
    (valueFirstUse ptr).get ctx (by grind) = ptr.getFirstUse ctx := by
  grind [get]

@[simp, grind =]
theorem get!_valueFirstUse_eq {ptr : ValuePtr} {ctx : IRContext dT} :
    (valueFirstUse ptr).get! ctx = ptr.getFirstUse! ctx := by
  grind [get!]

def set (ptrPtr : OpOperandPtrPtr) (ctx : IRContext dT) (newValue : Option OpOperandPtr) (ptrPtrIn : ptrPtr.InBounds ctx := by grind) : IRContext dT :=
  match ptrPtr with
  | operandNextUse ptr =>
...
def set! (ptrPtr : OpOperandPtrPtr) (ctx : IRContext dT) (newValue : Option OpOperandPtr) : IRContext dT :=
  match ptrPtr with
  | operandNextUse ptr =>
    ptr.setNextUse! ctx newValue
  | valueFirstUse val =>
...
@[simp]
theorem set_operandNextUse (ptr : OpOperandPtr) (ctx : IRContext dT) (newValue : Option OpOperandPtr) (ptrIn : (operandNextUse ptr).InBounds ctx) :
    (operandNextUse ptr).set ctx newValue ptrIn = ptr.setNextUse ctx newValue := by
  unfold set; rfl

@[simp]
theorem set_valueFirstUse (ptr : ValuePtr) (ctx : IRContext dT) (ptrIn : (valueFirstUse ptr).InBounds ctx) (newValue : Option OpOperandPtr) :
...
def InBounds (region : RegionPtr) (ctx : IRContext dT) : Prop :=
  region ∈ ctx.regions

def get (ptr : RegionPtr) (ctx : IRContext dT) (inBounds : ptr.InBounds ctx := by grind) : Region :=
  ctx.regions[ptr]'(by unfold InBounds at inBounds; grind)

def get! (ptr : RegionPtr) (ctx : IRContext dT) : Region := ctx.regions[ptr]!
...
def set (ptr : RegionPtr) (ctx : IRContext dT) (newRegion : Region) : IRContext dT :=
  {ctx with regions := ctx.regions.insert ptr newRegion}

def setParent (region : RegionPtr) (ctx : IRContext dT) (newParent : OperationPtr)
    (inBounds : region.InBounds ctx := by grind) : IRContext dT :=
  let oldRegion := region.get ctx (by grind)
  region.set ctx { oldRegion with parent := newParent}

def setParent! (region : RegionPtr) (ctx : IRContext dT) (newParent : OperationPtr) : IRContext dT :=
...
def setFirstBlock (region : RegionPtr) (ctx : IRContext dT) (newFirstBlock : Option BlockPtr)
    (inBounds : region.InBounds ctx := by grind) : IRContext dT :=
  let oldRegion := region.get ctx (by grind)
  region.set ctx { oldRegion with firstBlock := newFirstBlock}

def setFirstBlock! (region : RegionPtr) (ctx : IRContext dT) (newFirstBlock : Option BlockPtr) : IRContext dT :=
  let oldRegion := region.get! ctx
  region.set ctx {oldRegion with firstBlock := newFirstBlock}
...
def setLastBlock (region : RegionPtr) (ctx : IRContext dT) (newLastBlock : Option BlockPtr)
    (inBounds : region.InBounds ctx := by grind) : IRContext dT :=
  let oldRegion := region.get ctx (by grind)
  region.set ctx { oldRegion with lastBlock := newLastBlock}

def setLastBlock! (region : RegionPtr) (ctx : IRContext dT) (newLastBlock : Option BlockPtr) : IRContext dT :=
  let oldRegion := region.get! ctx
  region.set ctx {oldRegion with lastBlock := newLastBlock}
...
def allocEmpty (ctx : IRContext dT) : Option (IRContext dT × RegionPtr) :=
...
@[local grind]
inductive InBounds (ctx : IRContext dT) : BlockOperandPtrPtr → Prop
...
@[simp, grind=]
theorem inBounds_operandNextUse (ptr : BlockOperandPtr) (ctx : IRContext dT) :
    (blockOperandNextUse ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind

@[simp, grind=]
theorem inBounds_valueFirstUse (ptr : BlockPtr) (ctx : IRContext dT) :
    (blockFirstUse ptr).InBounds ctx ↔ ptr.InBounds ctx := by
  grind

def get (ptrPtr : BlockOperandPtrPtr) (ctx : IRContext dT) (ptrPtrIn : ptrPtr.InBounds ctx := by grind) : Option BlockOperandPtr :=
  match ptrPtr with
  | blockOperandNextUse ptr => (ptr.get ctx).nextUse
  | blockFirstUse val => (val.get ctx (by grind)).firstUse

def get! (ptrPtr : BlockOperandPtrPtr) (ctx : IRContext dT) : Option BlockOperandPtr :=
  match ptrPtr with
  | blockOperandNextUse ptr => (ptr.get! ctx).nextUse
...
def set (ptrPtr : BlockOperandPtrPtr) (ctx : IRContext dT) (newValue : Option BlockOperandPtr) (ptrPtrIn : ptrPtr.InBounds ctx := by grind) : IRContext dT :=
  match ptrPtr with
  | blockOperandNextUse ptr => ptr.setNextUse ctx newValue
  | blockFirstUse val => val.setFirstUse ctx newValue

def set! (ptrPtr : BlockOperandPtrPtr) (ctx : IRContext dT) (newValue : Option BlockOperandPtr) :
    IRContext dT :=
  match ptrPtr with
  | blockOperandNextUse ptr => ptr.setNextUse! ctx newValue
  | blockFirstUse val => val.setFirstUse! ctx newValue
...
def hasUses.loop (op : OperationPtr) (ctx : IRContext dT) (index : Nat)
...
def hasUses (op : OperationPtr) (ctx : IRContext dT) (opIn : op.InBounds ctx := by grind) : Bool :=
...
def hasUses!.loop (op : OperationPtr) (ctx : IRContext dT) (index : Nat) : Bool :=
  if ((op.getResult index).get! ctx).firstUse.isSome then
    true
  else
    match index with
...
def hasUses! (op : OperationPtr) (ctx : IRContext dT) : Bool :=
  let numResults := op.getNumResults! ctx
...
theorem hasUses!.loop_eq_hasUses_loop {op : OperationPtr} (ctx : IRContext dT) (index : Nat)
...
def IRContext.empty (dT : Type) [HasProperties dT] : IRContext dT := {
    nextID := 0,
...
-/
def IRContext.forOpsDepM (ctx : IRContext dT) {m : Type w → Type w'} [Monad m]
...
def InBounds (ptr : GenericPtr) (ctx : IRContext dT) : Prop :=
  match ptr with
