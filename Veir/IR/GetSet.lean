...
theorem BlockPtr.get!_OperationPtr_allocEmpty {block : BlockPtr} :
  block.get! (OperationPtr.allocEmpty ctx ty properties).1 = block.get! ctx := by
  grind

@[grind =]
theorem OperationPtr.get!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.get! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      Operation.empty ty properties
    else
...
theorem OperationPtr.getOpType!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getOpType! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      ty
    else
...
theorem OperationPtr.getNumResults!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getNumResults! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      0
    else
...
theorem OpResultPtr.get!_OperationPtr_allocEmpty {opResult : OpResultPtr} :
  opResult.get! (OperationPtr.allocEmpty ctx ty properties).1 = opResult.get! ctx := by
  have := ctx.nextID_free
  grind [Operation.default_results_eq]

@[grind =]
theorem OperationPtr.getNumOperands!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getNumOperands! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      0
    else
...
theorem OpOperandPtr.get!_OperationPtr_allocEmpty  {opOperand : OpOperandPtr} :
  opOperand.get! (OperationPtr.allocEmpty ctx ty properties).1 = opOperand.get! ctx := by
  have := ctx.nextID_free
  grind [Operation.default_operands_eq]

@[simp, grind =]
theorem OperationPtr.getProperties!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getProperties! (OperationPtr.allocEmpty ctx ty properties).1 ty =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      properties
    else
...
theorem OperationPtr.getOperands!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getOperands! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then
      #[]
    else
...
theorem OperationPtr.getNumSuccessors!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getNumSuccessors! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then 0 else operation.getNumSuccessors! ctx := by
  grind

@[simp, grind =]
theorem BlockOperandPtr.get!_OperationPtr_allocEmpty {blockOperand : BlockOperandPtr} :
    blockOperand.get! (OperationPtr.allocEmpty ctx ty properties).1 = blockOperand.get! ctx := by
  have := ctx.nextID_free
...
theorem OperationPtr.getNumRegions!_OperationPtr_allocEmpty {operation : OperationPtr} :
    operation.getNumRegions! (OperationPtr.allocEmpty ctx ty properties).1 =
    if operation = (OperationPtr.allocEmpty ctx ty properties).2 then 0 else operation.getNumRegions! ctx := by
  grind

@[simp, grind =]
theorem OperationPtr.getRegion!_OperationPtr_allocEmpty  {operation : OperationPtr} :
    operation.getRegion! (OperationPtr.allocEmpty ctx ty properties).1 i =
...
theorem BlockOperandPtrPtr.get!_OperationPtr_allocEmpty {blockOperandPtr : BlockOperandPtrPtr} :
    blockOperandPtr.get! (OperationPtr.allocEmpty ctx ty properties).1 = blockOperandPtr.get! ctx := by
  grind

@[simp, grind =]
theorem BlockPtr.getNumArguments!_OperationPtr_allocEmpty {block : BlockPtr} :
    block.getNumArguments! (OperationPtr.allocEmpty ctx ty properties).1 = block.getNumArguments! ctx := by
  grind

@[simp, grind =]
theorem BlockArgumentPtr.get!_OperationPtr_allocEmpty {blockArg : BlockArgumentPtr} :
    blockArg.get! (OperationPtr.allocEmpty ctx ty properties).1 = blockArg.get! ctx := by
  grind

@[simp, grind =]
theorem RegionPtr.get!_OperationPtr_allocEmpty {region : RegionPtr} :
    region.get! (OperationPtr.allocEmpty ctx ty properties).1 = region.get! ctx := by
  grind

@[simp, grind =]
theorem ValuePtr.getFirstUse!_OperationPtr_allocEmpty {value : ValuePtr} :
  value.getFirstUse! (OperationPtr.allocEmpty ctx ty properties).1 = value.getFirstUse! ctx := by
  grind

@[simp, grind =]
theorem ValuePtr.getType!_OperationPtr_allocEmpty {value : ValuePtr} :
  value.getType! (OperationPtr.allocEmpty ctx ty properties).1 = value.getType! ctx := by
  grind

@[simp, grind =]
theorem OpOperandPtrPtr.get!_OperationPtr_allocEmpty {opOperandPtr : OpOperandPtrPtr} :
  opOperandPtr.get! (OperationPtr.allocEmpty ctx ty properties).1 = opOperandPtr.get! ctx := by
  grind
