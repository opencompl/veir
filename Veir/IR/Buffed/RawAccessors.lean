...
@[inline]
def OpOperandPtrMPtr.write (operandPtr : OpOperandPtrMPtr) (val : OpOperandMPtr)
...
@[inline]
def BlockOperandPtrMPtr.write (operandPtr : BlockOperandPtrMPtr) (val : BlockOperandMPtr)
    (h : operandPtr.toNat + Buffed.ptrSize.toNat ≤ bctx.size) : IRBufContext OpInfo :=
  { bctx with mem := bctx.mem.blit64 operandPtr val (by grind) }
