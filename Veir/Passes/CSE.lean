...
/- This is the basis for CSE: if two instructions have the same Key,
   then they compute the same value: they are interchangeable. In that
   case we can remove one of them and switch all of its uses to the
   other. Proving this will be the crux of the eventual correctness
   proof for this pass. -/
structure Key where
  kind : Kind
  resultType : TypeAttr
...
/- Because ValuePtr is a sum type where the numeric IDs assigned to an
   opResult and a blockArgument might overlap, we add a separate tag
   to discriminate them reliably for purposes of computing a total
   order -/
...
/- Compute the Key for a commutative binary instruction -/
...
/- Compute the Key for a generic instruction -/
def ordinaryKey
    (ctx : IRContext OpCode) (op : OperationPtr) (kind : Kind) :
    Key :=
...
/- Perform CSE on a single BB: Walk the operations, building up a list
   of available values. For any operation whose value is already
...
def processAllBlocks (ctx : WfIRContext OpCode) :
  ...
  for block in ctx.raw.blocks.keys do
    if block.InBounds ctx.raw then
