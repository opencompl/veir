...
| `RewriteProofs/LowerSlti.lean` | `slti_local_preservesSemantics` (immediate `icmp x C` on `i64` → `slti`/`sltiu` (`slt`/`ult`), `xori (slti/sltiu) 1` (`sge`/`uge`), and the `x ≤ C ≡ x < C+1` off-by-one form `slti/sltiu (C+1)` (`sle`/`ule`, `ule` guarded on `C ≠ -1`); first `i1`-result lowering, 6-way predicate dispatch, 3-op and 4-op emit chains) + `matchIcmp_interpretOp_unfold` (dynamic-width, fixed-`i1` unfold) + `IsVerifiedICmp`/`Verified.llvm_icmp` verifier bundle + six `slti_*`/`sltiu_*` data lemmas + `#guard_msgs` axiom pin. **Tooling:** the predicate-dispatch `match` must be reduced by `split` (not `simp only []`/`dsimp`, which unfold `getType!`/`Option.bind` and corrupt the term); the emit chain is then peeled with `rw [Option.bind_eq_some_iff]`-based extraction (composes with `split`, unlike the `simp`-based `peel*` macros), factored into the file-local `sltiNonWrapArm`/`sltiWrapArm` macros | — (one-off; the `split`+`rw` arm macros generalise to future predicate-dispatch lowerings) |
...
| `Veir/Verifier.lean` | `IsVerified*` bundles (`…IntegerUnop`/`Binop`/`Ternop`/`LLVMShift`/`Select`/`ICmp`) + `Verified.llvm_*` extractors (Layer 1; incl. `Verified.llvm_mlir__constant`, `Verified.llvm_shl`/`llvm_lshr`, `Verified.llvm_select`, `Verified.llvm_icmp`) | one 3-line lemma (unop/binop/ternop/shift/select/icmp) |
