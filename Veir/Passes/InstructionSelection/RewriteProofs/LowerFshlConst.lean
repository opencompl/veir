...
/-!
## Correctness of `fshlConst_local`

`llvm.intr.fshl a a (const)` with identical data operands and a constant shift amount is a constant
rotate-left. There is no `roli`/`roliw`, so (like LLVM) a rotate-left by `amt` is emitted as a
rotate-right by `width - amt`: on `i64` it is lowered to `unrealized_conversion_cast` (value →
register) → `riscv.rori` (immediate form, negated amount) → `unrealized_conversion_cast` (back to
`i64`); on `i32` the exact `roriw` shape of `roliw_local` (negated immediate).

This is the width-generic sibling of `roliw_local`: everything up to and including the value cast is
shared; the lowering then branches on `bitwidth = 32` (emitting `roriw`, whose refinement is
`fshl_roriw_const_isRefinedBy_toInt`) vs the `i64` case (emitting `rori`, refined by
`fshl_rori_const_isRefinedBy_toInt`). Both data-level refinements are discharged directly by
`veir_bv_decide` after normalizing the (negated) immediate to the low bits of the constant.
-/

/-- The data-level rotate refinement for the constant-amount i32 `fshl` lowering: a rotate-left by
    the constant `v` refines `roriw a, ((32 - v) mod 32)` (five-bit negated immediate). The i32
    analogue of `fshl_rori_const_isRefinedBy_toInt`. -/
theorem fshl_roriw_const_isRefinedBy_toInt {x xt : Data.LLVM.Int 32} (v : Int) (hx : x ⊒ xt) :
    Data.LLVM.Int.fshl x x (Data.LLVM.Int.constant 32 v)
      ⊒ RISCV.Reg.toInt
          (Data.RISCV.roriw (BitVec.ofInt 5 (-(v % 32) % 32)) (LLVM.Int.toReg xt)) 32 := by
  rw [Data.RISCV.roriw,
    show BitVec.ofInt 5 (-(v % 32) % 32) = -(BitVec.setWidth 5 (BitVec.ofInt 32 v)) from by
      rw [← BitVec.toNat_inj]
...
/-- The data-level rotate refinement for the constant-amount i64 `fshl` lowering: a rotate-left by
    the constant `v` refines `rori a, ((64 - v) mod 64)`, where the operand refinement `x ⊒ xt`
    threads the (possibly-more-defined) target-side value. The i64 analogue of
    `fshl_roriw_const_isRefinedBy_toInt` (six-bit negated immediate); the `rori`-sibling of
    `fshr_rori_const_isRefinedBy_toInt` with the immediate negated mod 64. -/
theorem fshl_rori_const_isRefinedBy_toInt {x xt : Data.LLVM.Int 64} (v : Int) (hx : x ⊒ xt) :
    Data.LLVM.Int.fshl x x (Data.LLVM.Int.constant 64 v)
      ⊒ RISCV.Reg.toInt
          (Data.RISCV.rori (BitVec.ofInt 6 (-(v % 64) % 64)) (LLVM.Int.toReg xt)) 64 := by
  rw [Data.RISCV.rori,
    show BitVec.ofInt 6 (-(v % 64) % 64) = -(BitVec.setWidth 6 (BitVec.ofInt 64 v)) from by
      rw [← BitVec.toNat_inj]
      simp only [BitVec.toNat_setWidth, BitVec.toNat_ofInt, BitVec.toNat_neg, Nat.reducePow]; omega]
  simp only [Data.LLVM.Int.constant]
...
/-- Correctness of the `fshlConst_local` lowering: the `castToReg → rori/roriw (negated imm) →
    castBack` round trip refines `llvm.intr.fshl a a (const)` on `i64`/`i32`. -/
theorem fshlConst_local_preservesSemantics :
    LocalRewritePattern.PreservesSemantics fshlConst_local h h₂ h₃ h₄ := by
  simp only [LocalRewritePattern.PreservesSemantics, fshlConst_local]
  ...
  -- Unfold the interpretation of the matched `fshl`: exposes the operand values and their `fshl`.
  ...
  subst hCf
  -- `a = b` gives `xa = xb`, so the source result `fshl xa xb xc` is the rotate `fshl xa xa xc`.
  have hxab : xa = xb := by
  ...
  · -- i32: `roriw` (the `roliw_local` shape, negated immediate).
    subst hbw32
    ...
    have hRoriwProps : roriwOp.getProperties! ctx₃.raw (.riscv .roriw)
        = RISCVImmediateProperties.mk
            (IntegerAttr.mk (-(amtAttr.value % 32) % 32) (IntegerType.mk 64)) := by
    ...
    obtain ⟨s₂, hI₂, hMem₂, hRes₂, -⟩ :=
      interpretOp_riscv_unaryReg_imm_forward (state := s₁) (inBounds := by grind)
        (res := Data.RISCV.roriw (BitVec.ofInt 5 (-(amtAttr.value % 32) % 32))
          (LLVM.Int.toReg xt))
    ...
    · refine ⟨#[RuntimeValue.int 32 (RISCV.Reg.toInt
          (Data.RISCV.roriw (BitVec.ofInt 5 (-(amtAttr.value % 32) % 32))
            (LLVM.Int.toReg xt)) 32)], ?_, ?_⟩
      · simp [hRes₃, Option.bind, Option.map]
      · exact RuntimeValue.arrayIsRefinedBy_singleton.mpr
          ⟨rfl, by simpa using fshl_roriw_const_isRefinedBy_toInt amtAttr.value hxtRef⟩
  · -- i64: `rori` (negated immediate).
    ...
    have hRoriProps : roriOp.getProperties! ctx₃.raw (.riscv .rori)
        = RISCVImmediateProperties.mk
            (IntegerAttr.mk (-(amtAttr.value % 64) % 64) (IntegerType.mk 64)) := by
      rw [OperationPtr.getProperties!_WfRewriter_createOp_ne hCastBack (by grind)]
    ...
    obtain ⟨s₂, hI₂, hMem₂, hRes₂, -⟩ :=
      interpretOp_riscv_unaryReg_imm_forward (state := s₁) (inBounds := by grind)
        (res := Data.RISCV.rori (BitVec.ofInt 6 (-(amtAttr.value % 64) % 64))
          (LLVM.Int.toReg xt))
        (fun _ _ _ => by simp [Riscv.interpretOp', Interp, pure])
    ...
    · refine ⟨#[RuntimeValue.int 64 (RISCV.Reg.toInt
          (Data.RISCV.rori (BitVec.ofInt 6 (-(amtAttr.value % 64) % 64))
            (LLVM.Int.toReg xt)) 64)], ?_, ?_⟩
      · simp [hRes₃, Option.bind, Option.map]
      · exact RuntimeValue.arrayIsRefinedBy_singleton.mpr
          ⟨rfl, by simpa using fshl_rori_const_isRefinedBy_toInt amtAttr.value hxtRef⟩
