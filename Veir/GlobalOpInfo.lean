...
-- `OpCode.encode`/`decode`/`decode_encode` are generated by `#generate_op_codes`, which
-- assigns every registered operation a distinct code (0 decodes to `builtin.unregistered`).
