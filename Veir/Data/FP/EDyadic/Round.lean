...
These mirror `UnpackedFloat.blastExtract{GuardBit,StickyBit,IsEven}`
...
private def blastExtractGuardBit (absN : Nat) (b : Nat) : Bool :=
  if b = 0 then false else absN.testBit (b - 1)

/-- Sticky bit: any bit below position `b - 1` of `absN` is set. -/
private def blastExtractStickyBit (absN : Nat) (b : Nat) : Bool :=
  if b ≤ 1 then false
  else absN % (1 <<< (b - 1)) ≠ 0

/-- LSB of the truncated mantissa is zero. -/
private def blastExtractIsEven (absN : Nat) (b : Nat) : Bool :=
  ! absN.testBit b
...
private def needsRounding (absN : Nat) (b : Nat) : Bool :=
  blastExtractGuardBit absN b || blastExtractStickyBit absN b

/-! ## Round-toward-zero and successor-away-from-zero

Mirror `UnpackedFloat.blastRoundTowardZero` and
`UnpackedFloat.blastSuccessorAwayFromZero`. These work on the unsigned
magnitude `absN` and produce the unsigned magnitude of the result. -/

/-- Truncated magnitude: `absN >>> b`. -/
private def blastRoundTowardZeroMag (absN : Nat) (b : Nat) : Nat :=
  absN >>> b

/-- Successor of the truncated magnitude (one ULP away from zero). -/
private def blastSuccessorAwayFromZeroMag (absN : Nat) (b : Nat) : Nat :=
  blastRoundTowardZeroMag absN b + 1
...
-/
private def blastSuccessorAwayFromZeroNonnegAuxMag (absN : Nat) (b : Nat) : Nat :=
  if needsRounding absN b then blastSuccessorAwayFromZeroMag absN b
  else blastRoundTowardZeroMag absN b
...
Mirror `UnpackedFloat.blastLower{Nonneg,Neg}` and
`UnpackedFloat.blastUpper{Nonneg,Neg}`. For `x ≥ 0` the bracketing
...
private def blastLowerNonneg (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  magToDyadic false (blastRoundTowardZeroMag absN b) prec

/-- For `x ≥ 0`: upper = successor-or-self of the truncated magnitude. -/
private def blastUpperNonneg (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  magToDyadic false (blastSuccessorAwayFromZeroNonnegAuxMag absN b) prec

/-- For `x < 0`: lower = `-upperNonneg`. -/
private def blastLowerNeg (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  -(blastUpperNonneg absN b prec)

/-- For `x < 0`: upper = `-lowerNonneg`. -/
private def blastUpperNeg (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  -(blastLowerNonneg absN b prec)

/-- Sign-aware lower of `x = ±absN * 2^(-k)`. -/
private def blastLower (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  if sign then blastLowerNeg absN b prec else blastLowerNonneg absN b prec

/-- Sign-aware upper of `x = ±absN * 2^(-k)`. -/
private def blastUpper (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  if sign then blastUpperNeg absN b prec else blastUpperNonneg absN b prec

/-! ## Position predicates: lower-half, tie-break, even

Mirror `UnpackedFloat.blastIs{LowerHalf,TieBreak,Even}` and the
`Lower`/`Upper` parity helpers. -/

/-- Nonneg `x`: x is in the lower half iff guard bit is zero. -/
private def blastIsLowerHalfNonneg (absN : Nat) (b : Nat) : Bool :=
  ! blastExtractGuardBit absN b

/--
Neg `x`: x is in the lower half (i.e., closer to the more-negative
bracket) iff guard bit is one. This mirrors
`uf.neg.blastExtractGuardBit` in the unpacked-float setting.
-/
private def blastIsLowerHalfNeg (absN : Nat) (b : Nat) : Bool :=
  blastExtractGuardBit absN b

/-- Sign-aware "x is in the lower half of `[lower, upper]`". -/
private def blastIsLowerHalf (sign : Bool) (absN : Nat) (b : Nat) : Bool :=
  if sign then blastIsLowerHalfNeg absN b else blastIsLowerHalfNonneg absN b

/--
Tie-break: `x` is exactly at the midpoint. This is symmetric across
sign: guard bit set with no sticky bits below.
-/
private def blastIsTieBreak (_sign : Bool) (absN : Nat) (b : Nat) : Bool :=
  blastExtractGuardBit absN b && ! blastExtractStickyBit absN b

/-- Whether the *lower* bracket's mantissa is even (sign-aware). -/
private def blastIsEven (sign : Bool) (absN : Nat) (b : Nat) : Bool :=
  if sign then ! blastExtractIsEven absN b
  else blastExtractIsEven absN b

/-- Whether the lower bracket's mantissa is even. -/
private def blastIsEvenLower (sign : Bool) (absN : Nat) (b : Nat) : Bool :=
  blastIsEven sign absN b

/-- Whether the upper bracket's mantissa is even. -/
private def blastIsEvenUpper (sign : Bool) (absN : Nat) (b : Nat) : Bool :=
  ! blastIsEven sign absN b

/-! ## Per-mode rounding (mirrors `blastSmtLibRound{RNE,RNA,RTP,RTN,RTZ}`)

Each `blastSmtLibRound*` returns the rounded `Dyadic`. Overflow on the
result (and the input early-overflow case) is handled by the
public-facing `round`. -/

/-- Round-to-nearest, tie to even. -/
private def blastSmtLibRoundRNE
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  if ! blastIsLowerHalf sign absN b && ! blastIsTieBreak sign absN b then
    blastUpper sign absN b prec
  else if blastIsTieBreak sign absN b && blastIsEvenUpper sign absN b then
    blastUpper sign absN b prec
  else if blastIsTieBreak sign absN b && blastIsEvenLower sign absN b then
    blastLower sign absN b prec
  else if blastIsLowerHalf sign absN b then
    blastLower sign absN b prec
  else
...
private def blastSmtLibRoundRNA
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  if sign = false && ! blastIsLowerHalf sign absN b then
    blastUpper sign absN b prec
  else if sign = false && blastIsLowerHalf sign absN b then
    blastLower sign absN b prec
  else if sign = true && ! blastIsLowerHalf sign absN b
        && ! blastIsTieBreak sign absN b then
    blastUpper sign absN b prec
  else if sign = true
        && (blastIsLowerHalf sign absN b || blastIsTieBreak sign absN b) then
    blastLower sign absN b prec
  else
    blastLower sign absN b prec

/-- Round toward `+∞`: pick the upper bracket. -/
private def blastSmtLibRoundRTP
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  blastUpper sign absN b prec

/-- Round toward `−∞`: pick the lower bracket. -/
private def blastSmtLibRoundRTN
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  blastLower sign absN b prec

/-- Round toward zero: pick the bracket whose magnitude is smaller. -/
private def blastSmtLibRoundRTZ
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  if sign then blastUpper sign absN b prec
  else blastLower sign absN b prec

/--
Dispatcher: round per the given mode. Mirrors
`UnpackedFloat.blastSmtLibRoundAux`.
-/
private def blastSmtLibRoundAux (mode : RoundingMode)
    (sign : Bool) (absN : Nat) (b : Nat) (prec : Int) : Dyadic :=
  match mode with
  | .RNE => blastSmtLibRoundRNE sign absN b prec
  | .RNA => blastSmtLibRoundRNA sign absN b prec
  | .RTP => blastSmtLibRoundRTP sign absN b prec
  | .RTN => blastSmtLibRoundRTN sign absN b prec
  | .RTZ => blastSmtLibRoundRTZ sign absN b prec

/-! ## Overflow specials

Mirror `blastRounderSpecialCaseOverflow`. -/
    ...
    if eVal > maxEx then
      ...
      if shift ≤ 0 then
        ...
        wrapResult .RTN sign e s
          (blastLower sign n.natAbs shift.toNat prec)
    ...
    if eVal > maxEx then
      ...
      if shift ≤ 0 then
        ...
        wrapResult .RTP sign e s
          (blastUpper sign n.natAbs shift.toNat prec)

/--
Round `x` per IEEE-754 mode `mode` in target format `(e, s)`.

Mirrors `UnpackedFloat.blastSmtLibRound`:
...
* Otherwise we round at the precision dictated by `x`'s exponent
  class, via the per-mode `blastSmtLibRound*` family.
-/
    ...
    if eVal > maxEx then
      ...
      if shift ≤ 0 then
        ...
        wrapResult mode sign e s
          (blastSmtLibRoundAux mode sign n.natAbs shift.toNat prec)
