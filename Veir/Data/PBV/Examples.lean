...
-- Step 5B: Translate the width precondition `w ≤ 4` into `mw ≤ BitVec.ofNat 4 (2 ^ 4 - 1)`
  have bv_hw := le_of_le_of_eq_maskOfWidth w_le_bw (by decide) h_mw (maskOfWidth.eq_1 4 4).symm hw
...
--          into a fact about the bitvector masks
  have bv_hr := le_of_le_of_eq_maskOfWidth r_le_bw (by decide) h_mr (maskOfWidth.eq_1 8 8).symm hr
...
-- Step 5B: Translate the width precondition `w ≤ 8` into `mw ≤ BitVec.ofNat 16 (2 ^ 8 - 1)`
  have bv_hw := le_of_le_of_eq_maskOfWidth w_le_bw (by decide) h_mw (maskOfWidth.eq_1 16 8).symm hw
-- Step 6: Remove natural numbers from goal and hyps, by pushing setWidths down.
  simp only [
    eq_iff (o := 16),
    setWidth_add,
