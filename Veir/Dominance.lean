...
-/
axiom OperationPtr.properlyDominates_of_getDefiningOp!_of_mem_getOperands! (ctxDom : ctx.Dom) :
  value.getDefiningOp! ctx.raw = some op₁ →
  value ∈ op₂.getOperands! ctx.raw →
  op₁.ProperlyDominates op₂ ctx true

grind_pattern OperationPtr.properlyDominates_of_getDefiningOp!_of_mem_getOperands! =>
