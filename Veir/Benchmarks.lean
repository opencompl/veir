...
def addIConstantFolding (rewriter: PatternRewriter OpCode) (op: OperationPtr) (_ : op.InBounds rewriter.ctx.raw) : Option (PatternRewriter OpCode) := do
  ...
  let newVal := ArithConstantProperties.mk (IntegerAttr.mk (lhsVal + rhsVal) ({ bitwidth := 32 }))
  let (rewriter, newOp) ← rewriter.createOp (.arith .constant) #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] newVal (some $ .before op) sorry sorry sorry sorry
...
def addIConstantFoldingLocal (ctx: WfIRContext OpCode) (op: OperationPtr) :
  ...
  let newVal := ArithConstantProperties.mk (IntegerAttr.mk (lhsVal + rhsVal) ({ bitwidth := 32 }))
  let (ctx, newOp) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] newVal none sorry sorry sorry sorry
...
def mulITwoReduce (rewriter: PatternRewriter OpCode) (op: OperationPtr) (_ : op.InBounds rewriter.ctx.raw) : Option (PatternRewriter OpCode) := do
  ...
  let (rewriter, newOp) ← rewriter.createOp (.arith .addi) #[({ bitwidth := 32 } : IntegerType)] #[lhsValuePtr, lhsValuePtr] #[] #[] (ArithIntegerOverflowFlagsProperties.mk { nsw := false, nuw := false }) (some $ .before op) sorry sorry sorry sorry
  let mut rewriter ← rewriter.replaceOp op newOp sorry sorry sorry sorry sorry
...
def addIConstantFolding (ctx: WfIRContext OpCode) (op: OperationPtr) : Option (WfIRContext OpCode) := do
  ...
  let rhsVal := (rhsOp.getProperties! ctx.raw Arith.constant).value.value
  let newVal := ArithConstantProperties.mk (IntegerAttr.mk (lhsVal + rhsVal) ({ bitwidth := 32 }))
  let (ctx, newOp) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] newVal (some $ .before op) sorry sorry sorry sorry
...
def mulITwoReduce (ctx: WfIRContext OpCode) (op: OperationPtr) : Option (WfIRContext OpCode) := do
  ...
  let (ctx, newOp) ← WfRewriter.createOp ctx Arith.addi #[({ bitwidth := 32 } : IntegerType)] #[lhsValuePtr, lhsValuePtr] #[] #[] (ArithIntegerOverflowFlagsProperties.mk { nsw := false, nuw := false }) (some $ .before op) sorry sorry sorry sorry
  let mut ctx ← WfRewriter.replaceOp? ctx op newOp sorry sorry sorry sorry sorry
...
def constFoldTree (opcode: OpCode) (prop : propertiesOf opcode) (size pc: Nat) (root inc: Int) : Option (WfIRContext OpCode × OperationPtr) := do
  let root := ArithConstantProperties.mk (IntegerAttr.mk root ({ bitwidth := 32 }))
  let inc := ArithConstantProperties.mk (IntegerAttr.mk inc ({ bitwidth := 32 }))
  let (gctx, topOp, insertPoint) ← empty
  let mut (gctx, gacc) ← WfRewriter.createOp gctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] root insertPoint sorry sorry sorry sorry
  for i in [0:size] do
    let ⟨thisOp, prop⟩ : (op : OpCode) × propertiesOf op := if (i % 100 < pc) then ⟨opcode, prop⟩ else ⟨.arith .andi, ()⟩
    let (ctx, acc) := (gctx, gacc)
    let (ctx, rhsOp) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] inc insertPoint sorry sorry sorry sorry
    let lhsVal := acc.getResult 0
    let rhsVal := rhsOp.getResult 0
    let (ctx, acc) ← WfRewriter.createOp ctx thisOp #[({ bitwidth := 32 } : IntegerType)] #[lhsVal, rhsVal] #[] #[] prop insertPoint sorry sorry sorry sorry
    (gctx, gacc) := (ctx, acc)
...
def constFoldTreeSparse (opcode : OpCode) (prop : propertiesOf opcode) (size pc : Nat) (root inc : Int) : Option (WfIRContext OpCode × OperationPtr) :=
  Xoshiro256PP.run do
    let rootAttr := ArithConstantProperties.mk (IntegerAttr.mk root ({ bitwidth := 32 }))
    let incAttr := ArithConstantProperties.mk (IntegerAttr.mk inc ({ bitwidth := 32 }))
    let (gctx, topOp, insertPoint) ← empty

    let mut (gctx, root) ← WfRewriter.createOp gctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] rootAttr insertPoint sorry sorry sorry sorry
    ...
    while runningTotals.size < size do
      ...
      if const then
        let (ctx, op) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] incAttr insertPoint sorry sorry sorry sorry
        ...
        if let some lhs ← randIdx runningTotals then
          if let some rhs ← randIdx constants then
            ...
            let (ctx, op) ← WfRewriter.createOp ctx thisOp #[({ bitwidth := 32 } : IntegerType)] #[lhs, rhs] #[] #[] prop insertPoint sorry sorry sorry sorry
...
def constReuseTree (opcode: OpCode) (prop : propertiesOf opcode) (size pc: Nat) (root inc: Int) : Option (WfIRContext OpCode × OperationPtr) := do
  ...
  let (ctx, acc) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] root insertPoint sorry sorry sorry sorry
  let (ctx, reuse) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] inc insertPoint sorry sorry sorry sorry
...
def constLotsOfReuseTree (opcode: OpCode) (prop : propertiesOf opcode) (size pc: Nat) (lhs rhs: Int) : Option (WfIRContext OpCode × OperationPtr) := do
  let lhs := ArithConstantProperties.mk (IntegerAttr.mk lhs ({ bitwidth := 32 }))
  let rhs := ArithConstantProperties.mk (IntegerAttr.mk rhs ({ bitwidth := 32 }))
  let (ctx, topOp, insertPoint) ← empty
  let (ctx, lhsOp) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] lhs insertPoint sorry sorry sorry sorry
  let (ctx, rhsOp) ← WfRewriter.createOp ctx Arith.constant #[({ bitwidth := 32 } : IntegerType)] #[] #[] #[] rhs insertPoint sorry sorry sorry sorry
  let lhsVal := lhsOp.getResult 0
  let rhsVal := rhsOp.getResult 0
  let (ctx, reuse) ← WfRewriter.createOp ctx opcode #[({ bitwidth := 32 } : IntegerType)] #[lhsVal, rhsVal] #[] #[] prop insertPoint sorry sorry sorry sorry
