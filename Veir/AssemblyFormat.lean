...
`Format` AST, and the printer (`Veir/Printer.lean`) walks that AST. Parsing of
the custom syntax (a matching walk of the AST in `Veir/Parser/MlirParser.lean`)
is added in a follow-up; this module and PR only support *printing*. The set of
supported directives is the minimal subset needed for nice function syntax
(`func.return`, `func.call`); operations whose syntax cannot be expressed
declaratively (most notably `func.func`, which mirrors MLIR's
`hasCustomAssemblyFormat`) are handled by dedicated hooks instead.
