...
theorem RuntimeValue.isRefinedByUnder_id_refl (v : RuntimeValue) :
  ...
  cases v
  case addr p => cases p <;> simp [RuntimeValue.isRefinedByUnder]
  all_goals simp [RuntimeValue.isRefinedByUnder]
...
theorem RuntimeValue.isRefinedByUnder_id_iff {s t : RuntimeValue} :
  ...
  cases s <;> cases t <;>
    simp only [RuntimeValue.isRefinedByUnder, RuntimeValue.isRefinedBy]
  rename_i p q
  cases p <;> cases q <;> simp only [Ptr.isRefinedBy, id]
  /- The two pointers agree exactly when their object and offset do. -/
  rename_i a b
  cases a
  cases b
  simp
...
/-- Equal final memories with refining results refine as function results. -/
theorem FunctionResult.isRefinedBy_of_memory_eq {init mem : MemoryState}
    {vs vs' : Array RuntimeValue} (h : vs ⊒ vs') :
    FunctionResult.isRefinedBy init (mem, vs) (mem, vs') :=
...
@[simp, grind .]
theorem FunctionResult.isRefinedBy_refl (init : MemoryState)
    (r : MemoryState × Array RuntimeValue) : FunctionResult.isRefinedBy init r r :=
  ⟨id, fun _ _ => rfl, fun _ h => h,
    fun i hi _ => ⟨i, hi, rfl, MemoryObject.isRefinedByUnder_id_refl _⟩,
...
theorem RuntimeValue.isRefinedByUnder_trans {f g : Nat → Nat} {v₁ v₂ v₃ : RuntimeValue}
  ...
  cases v₁ <;> cases v₂ <;> cases v₃ <;>
    ...
    try grind [RuntimeValue.isRefinedBy, RuntimeValue.isRefinedBy_trans]
  rename_i p q r
  cases p <;> cases q <;> cases r <;> simp_all

theorem RuntimeValue.arrayIsRefinedByUnder_trans {f g : Nat → Nat} {a b c : Array RuntimeValue}
    (h12 : RuntimeValue.arrayIsRefinedByUnder f a b)
    (h23 : RuntimeValue.arrayIsRefinedByUnder g b c) :
