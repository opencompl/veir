...
theorem VariableState.setArgumentValues?_isRefinedByAt {ctx ctx' : WfIRContext OpCode}
  ...
  simp only [ValuePtr.inScopeAt_at] at hValDom hσValDom
  ...
  · rw [VariableState.getVar?_setArgumentValues?_of_notMem_getArguments! hMem hSrc] at hsv
    ...
    apply hRef val valIn
    · simpa only [ValuePtr.inScopeAt_blockEntry] using And.intro hValDom hMem
    · simpa only [ValuePtr.inScopeAt_blockEntry] using And.intro hσValDom hσnotMem
    · exact hsv
    · exact htv
...
theorem VariableState.getOperandValues_isRefinedByAt
  ...
  have hOpEq : ∀ (i : Nat) (hi : i < op.getNumOperands! ctx.raw),
    ...
    simpa only [OperationPtr.getOperands!.getElem_eq_getOperand!] using this
  refine ⟨by grind, ?_⟩
  intro i hi
...
theorem interpretOp_monotone_at
    ...
    (ctxDom : ctx.Dom) (ctxDom' : ctx'.Dom)
    (tgtDefDom : state'.DefinesDominating (.before op')
      (by simpa only [InsertPoint.inBounds_before] using opIn'))
    (opHasParent : (op.get! ctx.raw).parent = some block)
    (opHasParent' : (op'.get! ctx'.raw).parent = some block') :
    Interp.isRefinedBy
