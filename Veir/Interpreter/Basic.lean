...
def interpretOp' (opType : OpCode) (properties : HasOpInfo.propertiesOf opType)
  ...
  | .riscv_li => do
    let imm := BitVec.ofInt 64 properties.value.value.toNat
    return (#[.reg (RISCV.li imm)], .continue)
  | .riscv_lui => do
    let imm := BitVec.ofInt 20 properties.value.value.toNat
  ...
  | .riscv_addi => do
    ...
    let imm := BitVec.ofInt 12 properties.value.value.toNat
  ...
  | .riscv_slli => do
    let #[.reg op] := operands | none
    let imm := BitVec.ofInt 6 properties.value.value.toNat
