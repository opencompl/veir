...
def riscvEffectiveAddr (base : BitVec 64) (offset : BitVec 64) : BitVec 64 :=
  base + (offset.setWidth 12).signExtend 64
...
def Riscv.interpretOp' (opType : Veir.Riscv) (properties : propertiesOf opType)
  ...
  match opType with
  | .li => do
    let imm := properties.value.setWidth 64
    return (#[.reg (RISCV.li imm)], mem, none)
  | .lui => do
    let imm := properties.value.setWidth 20
  ...
  | .addi => do
    let [.reg op] := operands.toList | none
    let imm := properties.value.setWidth 12
  ...
  | .slli => do
    let [.reg op] := operands.toList | none
    let imm := properties.value.setWidth 6
  ...
  | .slliw => do
    ...
    let imm := properties.value.setWidth 5
  ...
  | .ld => do
    ...
    let eaddr := riscvEffectiveAddr addr.val properties.value
