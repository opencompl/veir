...
def Llvm.interpretOp' (opType : Veir.Llvm) (properties : propertiesOf opType)
  ...
  | .alloca => do
    ...
    let (mem, addr) ← MemoryModel.allocateObject mem properties.alignment.value.toNat (size * count.toNat)
  ...
  | .load => do
    ...
    let val ← MemoryModel.load mem type addr properties.alignment.value.toNat
    return (#[val], mem, none)
  | .store => do
    let [val, .addr addr] := operands.toList | none
    let .val addr := addr | Interp.ub
    let mem ← MemoryModel.store mem addr val properties.alignment.value.toNat
    return (#[], mem, none)
  | .getelementptr => do
  ...
  | .ptrtoint => do
    ...
    let .val q := p | return (#[.int bw.bitwidth .poison], mem, none)
    let .val a := MemoryModel.intFromPtr mem (.val q) | return (#[.int bw.bitwidth .poison], mem, none)
    /- A pointer turned into an integer has escaped. -/
    return (#[.int bw.bitwidth (.val (BitVec.ofNat bw.bitwidth a.toNat))], mem.escape q, none)
  | .inttoptr => do
...
def Riscv_Stack.interpretOp' (opType : Veir.Riscv_Stack) (properties : propertiesOf opType)
  ...
  match opType with
  | .alloca => do
    let (mem, addr) ← MemoryModel.allocateObject mem properties.alignment.toNat properties.size.toNat
