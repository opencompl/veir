...
`mkDefault`: Default state a fact starts in. Typically either bottom or top.
`propagate`: Hook that's called when the fact changes state. Typically used
             to enqueue a fact's dependents because it changed.
-/
...
`kind`: Tag to identify the implemented analysis.
`init`: Given the top level operation pointer, initializes the analysis to a valid state.
