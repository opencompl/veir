...
def mkDefault (anchor : LatticeAnchor) : ExecutableFact :=
  { anchor := anchor
    dependents := #[]
    payload := { live := false, subscribers := #[] } }

def propagate (state : ExecutableFact) (dfCtx : DataFlowContext)
...
def mkDefault (anchor : LatticeAnchor) : PredecessorFact :=
  { anchor := anchor
    dependents := #[]
    allKnown := true
    knownPredecessors := ∅
...
variable [FactSpec .executable] [SparseFactSpec .sparseConstant ConstantDomain]
...
def getOperandValues
  ...
  for i in [0:op.getNumOperands! irCtx] do
    ...
    dfCtx := dfCtx.modifyFact .sparseConstant (.ValuePtr operand) (fun fact =>
      SparseFact.useDefSubscribe fact kind)
    let latticeElement :=
...
def DeadCodeAnalysis [FactSpec .executable] [SparseFactSpec .sparseConstant ConstantDomain] : DataFlowAnalysis :=
