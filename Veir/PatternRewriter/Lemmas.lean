...
def interpretOpList' (ctx : IRContext OpCode) (list : List OperationPtr) (state : InterpreterState)
...
def LocalRewritePattern.PreservesSemantics (pattern : LocalRewritePattern OpCode) : Prop :=
  ∀ (ctx : IRContext OpCode) (op : OperationPtr) (opIn : op.InBounds ctx) newCtx newOps newValue,
