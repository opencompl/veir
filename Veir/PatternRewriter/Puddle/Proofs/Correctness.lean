...
private noncomputable def SemanticAssignment.ofConcrete
  ...
  exact fun id =>
    ...
    | some (some (.type type)) => some (SemanticBinding.type type)
    | some (some (.property opCode value)) =>
      some (SemanticBinding.property opCode value)
    | some (some (.value value)) =>
      match SemanticAssignment.ofConcreteValue state root rootValues value with
      | some runtimeValue => some (SemanticBinding.value runtimeValue)
      | none => none
    | _ => none
...
private theorem SemanticAssignment.ofConcrete_getProperty
  ...
  simp only [SemanticAssignment.ofConcrete]
...
private theorem SemanticAssignment.ofConcrete_getValue_eq_some
  ...
  have hslot : SemanticAssignment.ofConcrete assignment state root rootValues handle.id =
      some (.value runtimeValue) := by
    unfold SemanticAssignment.getValue at hget
    split at hget <;> simp_all
  simp only [SemanticAssignment.ofConcrete] at hslot
...
/-- A successful match resolves the root's individual result handles to its source results. -/
private theorem MatchProg.semantic_rootResults_of_run
    {prog : MatchProg OpCode α} {ctx : WfIRContext OpCode}
    {assignment : Assignment OpCode} {state : InterpreterState ctx}
    {root : OperationPtr} {rootValues : Array RuntimeValue}
    (hroot : prog.ConstrainsRoot) (hrun : prog.run ctx.raw root = some assignment)
    (hsize : (root.getResults! ctx.raw).size = rootValues.size) :
    prog.rootResults?.bind (fun handles =>
      (SemanticAssignment.ofConcrete assignment state root rootValues).getValues handles.toList) =
        some rootValues.toList := by
  obtain ⟨hrootGet, hpost⟩ := MatchProg.run_postconditions hrun
  cases hdecls : prog.decls with
  | nil => simp [MatchProg.ConstrainsRoot, hdecls] at hroot
  | cons decl rest =>
    cases decl with
    | operation opCode operands resultTypes property propertyHandle opHandle results resultsSize =>
      have heq : opHandle = prog.rootHandle := by
        simpa [MatchProg.ConstrainsRoot, hdecls] using hroot
      subst opHandle
      simp only [MatchProg.rootResults?, hdecls, ↓reduceIte, Option.bind_some]
      obtain ⟨before, after, hmatch, hext⟩ := hpost
        (.operation opCode operands resultTypes property propertyHandle prog.rootHandle
          results resultsSize) (by simp [hdecls])
      ...
      have hfinalOp := ((Assignment.Extends.bindValues hbindResults).trans
        hresultExtendsFinal).getOp (Assignment.bindOp_get hbindOp)
      have hopEq : operation = root := by
        rw [hrootGet] at hfinalOp
        exact (Option.some.inj hfinalOp).symm
      subst operation
      exact SemanticAssignment.ofConcrete_getValues_of_bindValues hbindResults hresultExtendsFinal
        (SemanticAssignment.ofConcreteValue_mapM_rootResults hsize)
    | value => simp [MatchProg.ConstrainsRoot, hdecls] at hroot
    | type => simp [MatchProg.ConstrainsRoot, hdecls] at hroot
    | applyNative => simp [MatchProg.ConstrainsRoot, hdecls] at hroot
...
private theorem MatchProg.models_of_run
    ...
    (hsourceValues : (root.getResults ctx.raw).mapM rootState.variables.getVar? =
      some sourceValues) :
    prog.ModelsWithAssignment
      (SemanticAssignment.ofConcrete assignment state root sourceValues) := by
  ...
  | operation opCode operands resultTypes property propertyHandle handle results _ =>
    ...
    have hsemanticOperands :
        (SemanticAssignment.ofConcrete assignment state root sourceValues).getValues
          operands.toList = some operandValues.toList :=
      hsemanticOperandsList
    ...
    have hsemanticResultTypes :
        (SemanticAssignment.ofConcrete assignment state root sourceValues).getTypes
          resultTypes.toList = some (operation.getResultTypes! ctx.raw).toList :=
      hsemanticResultTypesList
    ...
    have hresultConsistency :
        (SemanticAssignment.ofConcrete assignment state root sourceValues).getValues
          results.toList = some resultValues.toList :=
      hsemanticResultsList
    ...
    refine ⟨operandValues.toList, (operation.getResultTypes! ctx.raw).toList, resultValues,
      operation.getProperties! ctx.raw (operation.getOpType! ctx.raw),
      hsemanticOperands, hsemanticResultTypes, hsemanticProperty,
      hresultConsistency, hpropertyMatch, ?_⟩
    constructor
    · simpa using RuntimeValue.ArrayConforms_of_setResultValues?_eq_some hset
    intro targetMemory
    subst memory
...
private theorem Assignment.MetadataAgrees.bindOp
    ...
    (bound : Handle OpCode .op) (operation : OperationPtr)
    (hbind : Assignment.bindOp concrete bound operation = some after) :
    Assignment.MetadataAgrees after semantic := by
  have hext := Assignment.Extends.bindOp hbind
  constructor
  · intro query
    rw [← hagrees.1 query]
    ...
    · have hc : Assignment.getType after query = none := by
        ...
        rfl
      rw [hc]
      cases hbefore : Assignment.getType concrete query with
      | none => rfl
      | some ty =>
        have hafter := hext.getType hbefore
        simp [hc] at hafter
    · unfold Assignment.getType Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq]
  · intro opCode query
    rw [← hagrees.2 opCode query]
    by_cases heq : query.id = bound.id
    · have hc : Assignment.getProperty after query = none := by
        ...
        rfl
      rw [hc]
      cases hbefore : Assignment.getProperty concrete query with
      | none => rfl
      | some prop =>
        have hafter := hext.getProperty hbefore
        simp [hc] at hafter
    · unfold Assignment.getProperty Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq]
...
private theorem Assignment.MetadataAgrees.bindValue
  ...
  · intro query
    ...
    · unfold Assignment.bindValue at hbind
      ...
      rw [heq, Assignment.bind_get hbind, SemanticAssignment.bind_same_eq]
      rfl
    · unfold Assignment.bindValue at hbind
      unfold Assignment.getType Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindValue,
        SemanticAssignment.getType_bind_of_ne semantic bound.id (.value runtime) query heq]
  ...
  · intro queryOpCode query
    ...
    · unfold Assignment.bindValue at hbind
      unfold Assignment.getProperty Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindValue,
        SemanticAssignment.getProperty_bind_of_ne semantic bound.id (.value runtime) query heq]
  ...
  · intro query
    ...
    · unfold Assignment.bindType at hbind
      ...
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindType,
        SemanticAssignment.getType_bind_of_ne semantic bound.id (.type type) query heq]
      exact hagrees.1 query
  · intro queryOpCode query
    by_cases heq : query.id = bound.id
    · unfold Assignment.bindType at hbind
    ...
    · unfold Assignment.bindType at hbind
      unfold Assignment.getProperty Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindType,
        SemanticAssignment.getProperty_bind_of_ne semantic bound.id (.type type) query heq]
      exact hagrees.2 queryOpCode query
  ...
  · intro query
    ...
    · unfold Assignment.bindProperty at hbind
      ...
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindProperty,
        SemanticAssignment.getType_bind_of_ne semantic bound.id (.property opCode property) query heq]
      exact hagrees.1 query
  · intro queryOpCode query
    by_cases heq : query.id = bound.id
    · unfold Assignment.bindProperty at hbind
    ...
    · unfold Assignment.bindProperty at hbind
      unfold Assignment.getProperty Assignment.getBinding
      rw [Assignment.bind_get_of_ne hbind heq, SemanticAssignment.bindProperty,
        SemanticAssignment.getProperty_bind_of_ne semantic bound.id
          (.property opCode property) query heq]
      exact hagrees.2 queryOpCode query

private theorem Assignment.MetadataAgrees.bindValues
    {concrete after : Assignment OpCode} {semantic afterSemantic : SemanticAssignment}
    (hagrees : Assignment.MetadataAgrees concrete semantic)
    {handles : List (Handle OpCode .value)} {values : List ValuePtr}
    ...
    (hsemanticBind : semantic.bindValues handles runtimes = some afterSemantic) :
    Assignment.MetadataAgrees after afterSemantic := by
  induction handles generalizing concrete semantic after afterSemantic values runtimes with
  | nil =>
  ...
  | cons handle handles ih =>
    ...
    | cons value values =>
      ...
      | cons runtime runtimes =>
        ...
        simp only [SemanticAssignment.bindValues] at hsemanticBind
        exact ih (hagrees.bindValue handle value runtime hhead)
          (by simpa using hvalues) (by simpa using hruntimes) htail hsemanticBind
...
private theorem MetadataTuple.Atom.bind_agrees
    ...
    (hsemantic : atom.bindSemantic semantic handle value = semantic') :
  ...
  | type =>
    ...
    change semantic.bindType handle value = semantic' at hsemantic
    subst semantic'
  ...
  | property opCode =>
    ...
    change semantic.bindProperty handle value = semantic' at hsemantic
    subst semantic'
...
private theorem MetadataTuple.Shape.bind_agrees
  ...
  | cons head tail tailIH =>
    ...
    rw [Option.bind_eq_some_iff] at hconcrete
    obtain ⟨concreteMiddle, hcHead, hcTail⟩ := hconcrete
    exact tailIH (head.bind_agrees hagrees handles.1 values.1 hcHead rfl)
      handles.2 values.2 hcTail hsemantic
...
private theorem Assignment.Refines.bindOp
    ...
    (bound : Handle OpCode .op) (operation : OperationPtr)
    (hbind : Assignment.bindOp concrete bound operation = some after) :
    Assignment.Refines after semantic unavailable targetState := by
...
private theorem Assignment.Refines.bindType
  ...
  · rw [Assignment.getValue_bindType_ne bound query type hbind heq] at hconcrete
    rw [SemanticAssignment.bindType,
      SemanticAssignment.getValue_bind_of_ne semantic bound.id (.type type) query heq]
      at hsemantic
    exact hrefines query value sourceRuntime havailable hconcrete hsemantic
...
private theorem Assignment.Refines.bindProperty
  ...
  · rw [Assignment.getValue_bindProperty_ne bound query property hbind heq] at hconcrete
    rw [SemanticAssignment.bindProperty,
      SemanticAssignment.getValue_bind_of_ne semantic bound.id
...
private theorem Assignment.Refines.bindValue
  ...
  · rw [Assignment.getValue_bindValue_of_eq bound query value hbind heq] at hconcrete
    have hs : (semantic.bindValue bound sourceRuntime).getValue query =
      ...
      rw [heq, SemanticAssignment.bind_same_eq]
    rw [hs] at hsemantic
  ...
  · rw [Assignment.getValue_bindValue_of_ne bound query value hbind heq] at hconcrete
    rw [SemanticAssignment.bindValue,
      SemanticAssignment.getValue_bind_of_ne semantic bound.id (.value sourceRuntime) query heq]
      at hsemantic
...
private theorem Assignment.Refines.bindValues
    ...
    (hbind : Assignment.bindValues concrete handles values = some after)
    (hsemanticBind : semantic.bindValues handles sourceRuntimes = some afterSemantic) :
    Assignment.Refines after afterSemantic unavailable targetState := by
  induction handles generalizing concrete semantic after afterSemantic values sourceRuntimes
      targetRuntimes with
  | nil =>
    ...
    subst after
    simp [SemanticAssignment.bindValues] at hsemanticBind
    subst afterSemantic
    exact hrefines
  | cons handle handles ih =>
    cases values with
        ...
        | some target =>
          ...
          | some targets =>
            ...
            exact ih
              ...
              (by simpa using htargets) htail hrefinement'.2 hbindTail hsemanticBind
...
private theorem MetadataTuple.Shape.bind_refines
  ...
  | cons head tail tailIH =>
    ...
    rw [Option.bind_eq_some_iff] at hconcrete
    obtain ⟨concreteMiddle, hcHead, hcTail⟩ := hconcrete
    exact tailIH (head.bind_refines hrefines handles.1 values.1 hcHead rfl)
      handles.2 values.2 hcTail hsemantic
...
private theorem Assignment.Refines.getValuesList
  ...
  | nil =>
    simp at hconcrete
    subst values
    simp only [List.mapM_nil, pure, Option.some.injEq] at hsemantic
    subst sourceRuntimes
...
private theorem Assignment.Refines.getValues
    ...
    (hsemantic : SemanticAssignment.getValues semantic handles.toList = some sourceRuntimes) :
    ∃ targetRuntimes,
      values.mapM targetState.variables.getVar? = some targetRuntimes ∧
      sourceRuntimes.toArray ⊒ targetRuntimes := by
  ...
  have hsemanticList : handles.toList.mapM (SemanticAssignment.getValue semantic) =
      some sourceRuntimes := by
    simpa [SemanticAssignment.getValues] using hsemantic
...
private theorem CreateProg.checkBindingsDecls_unavailable
    ...
    (hcheck : decls.foldlM CreateDecl.checkBindings initial = some final) :
    final.unavailable = initial.unavailable := by
  induction decls generalizing initial with
  | nil =>
    simp only [List.foldlM_nil, pure, Option.some.injEq] at hcheck
    subst final
    rfl
  | cons decl decls ih =>
    simp only [List.foldlM_cons, Option.bind_eq_bind] at hcheck
    rw [Option.bind_eq_some_iff] at hcheck
...
private theorem CreateDecl.interpret
  ...
  | operation opCode operands resultTypes propertySource opHandle results =>
    ...
    · rename_i hoper
      ...
      have hsemanticResultTypes :
          semanticCreated.getTypes resultTypes.toList = some concreteResultTypes.toList := by
        have hgetType : Assignment.getType created = SemanticAssignment.getType semanticCreated :=
          funext hmetadata.1
        unfold SemanticAssignment.getTypes
        rw [← hgetType]
        simpa [Assignment.getTypes, Array.mapM_eq_mapM_toList] using
          congrArg (Option.map Array.toList) hconcreteResultTypes
      ...
      cases hsourceInterpret :
          interpretOp' opCode properties concreteResultTypes sourceOperands.toArray #[] .empty with
      ...
      | ok output =>
        ...
        | none =>
          ...
          have hevalBindings : semanticCreated.bindValues
              results.toList sourceResults.toList = some semanticCreated' := heval.2
          ...
          have hmonotone := interpretOp'_monotone opCode properties concreteResultTypes
            sourceOperands.toArray targetOperands #[] .empty hrefineOperands
          ...
          have hcreatedAfter :
            ...
            apply (hcreatedBaseAfter.bindOp opHandle newOp hbindOp).bindValues
              results.toList (newOp.getResults! ctx'.raw).toList
          ...
          have hmetadataAfter :
            ...
            apply Assignment.MetadataAgrees.bindValues
              (hmetadata.bindOp opHandle newOp hbindOp)
  ...
  | @applyNative Inputs Outputs inputBundle outputBundle inputs rewrite outputs =>
    ...
    · exact MetadataTuple.Shape.bind_refines
        (@IsMetadataTuple.shape OpCode inferInstance Outputs outputBundle)
        hcreated outputs outputValues hbind (by simpa [MetadataTuple.bindSemantic] using heval)
    · exact MetadataTuple.Shape.bind_agrees
        (@IsMetadataTuple.shape OpCode inferInstance Outputs outputBundle)
        hmetadata outputs outputValues hbind (by simpa [MetadataTuple.bindSemantic] using heval)
...
private theorem CreateProg.interpret_runDecls
    ...
    (hwellformed : decls.foldlM CreateDecl.checkBindings initialDefined = some finalDefined)
  ...
  | cons decl decls ih =>
    ...
    simp only [List.foldlM_cons, Option.bind_eq_bind] at hwellformed
  ...
  | some assignment =>
    ...
    | some created =>
      ...
      | some resolved =>
        ...
        have hdenotational : anyRewrite.PreservesSemanticsWithAssignment :=
          (Pattern.preservesSemantics_iff_withAssignment h.structurallyWellFormed).mp h.refines
        ...
        have hsemanticRoot := MatchProg.semantic_rootResults_of_run
          (state := state) (rootValues := sourceValues) hconstrainsRoot hrun
          (by
            have hs := Array.size_eq_of_mapM_eq_some hsourceValues
            simpa [OperationPtr.getResults!_eq_getResults rootIn] using hs)
        change anyRewrite.matcher.rootResults?.bind
          (fun handles => semanticMatched.getValues handles.toList) =
            some sourceValues.toList at hsemanticRoot
        ...
        · change
            anyRewrite.replacement.refinesRoot anyRewrite.matcher.rootResults?
              semanticMatched semanticCreated
          ...
          have hcheckDecls : anyRewrite.creation.decls.foldlM CreateDecl.checkBindings
          ...
          cases hsemanticReplacement :
              semanticCreated.getValues anyRewrite.replacement.values.toList with
          | none =>
            simp [Replacement.refinesRoot, hsemanticRoot,
              hsemanticReplacement] at hsemanticRefinement
          | some sourceReplacements =>
            have hSourceRefinement : sourceValues ⊒ sourceReplacements.toArray := by
