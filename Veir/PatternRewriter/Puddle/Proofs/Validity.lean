...
theorem MetadataTuple.bindSemantic_unit
    ...
    MetadataTuple.bindSemantic assignment handles values = some assignment := by
  rfl

@[simp]
theorem MetadataTuple.bindSemantic_type
    (assignment : SemanticAssignment) (handle : Handle OpCode .type) (value : TypeAttr) :
    MetadataTuple.bindSemantic assignment handle value = some (assignment.bindType handle value) := by
  rfl
  ...
  `(tactic| (
    ...
    · constructor
      · unfold $rule
        ...
        simp [MatchProg.Supported, MatchDecl.Supported, SupportedOpCode,
          ...
          MatchProg.operation, MatchProg.root, MatchProg.matchNative, bind, pure]
    ...
    · unfold $rule
      ...
      simp [MatchProg.ConstrainsRoot,
        Pattern.Builder, MatchProg.build, MatchProg.type, MatchProg.value,
        MatchProg.operation, MatchProg.root, MatchProg.matchNative,
        bind, pure]
      all_goals subst_vars <;> try simp_all
      all_goals try grind
    · unfold $rule
    ...
    simp (config := { maxSteps := 300000 })
      ...
      CreateProg.applyNative,
      CreateProg.evalDecls, CreateDecl.eval,
