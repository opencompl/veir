...
theorem InterpreterState.exists_getVar_of_result
    {ctx : WfIRContext OpCode} {operation : OperationPtr}
    {state after : InterpreterState ctx} {cf}
    (operationIn : operation.InBounds ctx.raw)
    {value : ValuePtr}
    (hvalue : value ∈ operation.getResults! ctx.raw)
    (hinterp : interpretOp operation state operationIn = .ok (after, cf)) :
    ∃ runtimeValue, after.variables.getVar? value = some runtimeValue := by
  obtain ⟨_, resultValues, _, variables, _, _, hset, rfl⟩ :=
    interpretOp_some_iff.mp hinterp
  simp only [OperationPtr.getResults!.mem_iff_exists_index] at hvalue
  obtain ⟨index, hindex, rfl⟩ := hvalue
  have hi : index < operation.getNumResults! ctx.raw := by
    simpa [OperationPtr.getResults!.size_eq_getNumResults!] using hindex
  exact ⟨resultValues[index]!, by
    simpa using VariableState.getVar?_getResult_of_setResultValues?
      (varState := state.variables) (varState' := variables) hi hset⟩
...
theorem Assignment.Rooted.exists_getValue
  ...
  rcases hoperand with hoperand | hresult
  ...
  · rcases hconsumer with rfl | hdominates
    · exact (hnotRootResult hresult).elim
    · have hdomIp : consumer.dominatesIp (InsertPoint.before root) ctx := by grind
      obtain ⟨consumerCf, consumerInterp⟩ := stateWf consumer consumerIn hpure hdomIp
      exact InterpreterState.exists_getVar_of_result consumerIn hresult consumerInterp
