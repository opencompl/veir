...
theorem HandleContext.insertFresh_unavailable
    {before after : HandleContext} {handle : Handle OpCode kind}
    (hinsert : before.insertFresh handle = some after) :
...
theorem HandleContext.insertManyFresh_unavailable
    {before after : HandleContext} {handles : List (Handle OpCode kind)}
    (hinsert : before.insertManyFresh handles = some after) :
    after.unavailable = before.unavailable := by
  ...
  | nil =>
    ...
    rfl
  | cons handle handles ih =>
    change (before.insertFresh handle).bind
      (fun middle => middle.insertManyFresh handles) = some after at hinsert
    rw [Option.bind_eq_some_iff] at hinsert
    obtain ⟨middle, hhead, htail⟩ := hinsert
    exact (ih htail).trans (HandleContext.insertFresh_unavailable hhead)
...
theorem MatchDecl.collectBindings_unavailable_mono
  ...
  | type matcher handle =>
    ...
    exact (HandleContext.insertFresh_unavailable hcollect) ▸ hmem
  | applyNative inputs predicate =>
  ...
  | operation opCode operands resultTypes property propertyHandle opHandle results matchOp =>
    ...
    rw [Option.bind_eq_some_iff] at hcollect
    obtain ⟨opCtx, hopInsert, hcollect⟩ := hcollect
    have hunavailable : opCtx.unavailable = before.unavailable :=
      (HandleContext.insertFresh_unavailable hopInsert).trans
        ((HandleContext.insertFresh_unavailable htypes).trans
          (HandleContext.insertManyFresh_unavailable hproperty))
    split at hcollect
    · change some ((opCtx.forbid opHandle).forbidMany results.toList) = some after at hcollect
      have heq := Option.some.inj hcollect
      subst after
      apply HandleContext.mem_unavailable_forbidMany
      simp [HandleContext.forbid, hunavailable, hmem]
    · change some opCtx = some after at hcollect
      have heq := Option.some.inj hcollect
      subst after
...
theorem MatchDecl.operation_collectBindings_rootAlias
  ...
  have hunavailable : opCtx.unavailable = before.unavailable :=
    (HandleContext.insertFresh_unavailable hopInsert).trans
      ((HandleContext.insertFresh_unavailable htypes).trans
        (HandleContext.insertManyFresh_unavailable hproperty))
  have halias' : opHandle.id ∈ opCtx.unavailable ∨
      ∃ result ∈ results.toList, result.id ∈ opCtx.unavailable := by
    simpa [hunavailable] using halias
  split at hcollect
  · change some ((opCtx.forbid opHandle).forbidMany results.toList) = some after at hcollect
    have heq := Option.some.inj hcollect
    subst after
...
theorem MatchDecl.propagateUnavailable_mono
    {decl : MatchDecl OpCode} {before after : HandleContext}
    (hpropagate : decl.propagateUnavailable before = after) :
    ∀ id ∈ before.unavailable, id ∈ after.unavailable := by
  intro id hmem
  cases decl with
  | operation opCode operands resultTypes property propertyHandle opHandle results matchOp =>
    simp only [MatchDecl.propagateUnavailable] at hpropagate
    split at hpropagate
    · subst after
      apply HandleContext.mem_unavailable_forbidMany
      simp [HandleContext.forbid, hmem]
    · subst after
      exact hmem
...
theorem MatchDecl.operation_propagateUnavailable_rootAlias
    {opCode : OpCode} {operands : Array (Handle OpCode .value)}
    {resultTypes : Array (Handle OpCode .type)} {property : PropertyMatcher opCode}
    {propertyHandle : Handle OpCode (.prop opCode)} {opHandle : Handle OpCode .op}
    {results : Array (Handle OpCode .value)}
    {resultsSize : results.size = resultTypes.size}
    {before after : HandleContext}
    (hpropagate : (MatchDecl.operation opCode operands resultTypes property propertyHandle
      opHandle results resultsSize).propagateUnavailable before = after)
    (halias : opHandle.id ∈ before.unavailable ∨
      ∃ result ∈ results.toList, result.id ∈ before.unavailable) :
    opHandle.id ∈ after.unavailable ∧
      ∀ result ∈ results.toList, result.id ∈ after.unavailable := by
...
theorem Assignment.RootAliasesUnavailable.runDecl
    ...
    (hpropagate : decl.propagateUnavailable defined = finalDefined) :
    after.RootAliasesUnavailable finalDefined root := by
  have hmono := MatchDecl.propagateUnavailable_mono hpropagate
  cases decl with
  | type matcher handle =>
  ...
  | operation opCode operands resultTypes property propertyHandle opHandle results matchOp =>
    ...
    have hforbidden (hfoundRoot : found = root) :=
      MatchDecl.operation_propagateUnavailable_rootAlias hpropagate (hrootAlias hfoundRoot)
...
theorem Assignment.RootAliasesUnavailable.runDecls
    ...
    (hpropagate : MatchProg.propagateUnavailable decls defined = finalDefined) :
    after.RootAliasesUnavailable finalDefined root := by
  induction decls generalizing before defined with
  | nil =>
    simp [MatchProg.runDecls] at hrun
    simp [MatchProg.propagateUnavailable] at hpropagate
    subst after
    subst finalDefined
    exact h
  | cons decl decls ih =>
    ...
    have hmiddleAliases := h.runDecl ctxDom hrooted hdecl rfl
    exact ih (fun query hmem => hsupported query (by simp [hmem]))
      hmiddleRooted hmiddleAliases hrest hpropagate
...
theorem MatchProg.rootResult_handle_unavailable_of_run
  ...
  simp only [pure, Option.some.injEq] at hcollect
  let initialDefined :=
    (rootDefined.forbid prog.rootHandle).forbidMany rootResults.toList
  ...
  have hinitialAliases : initial.RootAliasesUnavailable initialDefined root := by
    ...
    · intro query result hquery hresultRoot
      ...
      · have hnone :
        ...
        rw [hnone] at hempty
        contradiction
  have hfinalAliases := hinitialAliases.runDecls ctxDom
    (by simpa only [MatchProg.Supported] using hsupported) hinitialRooted
    hrun hcollect
...
theorem MatchProg.rooted_of_run
  ...
  exact Assignment.Rooted.runDecls ctxDom
    (by simpa only [MatchProg.Supported] using hsupported) hinitialRooted h
