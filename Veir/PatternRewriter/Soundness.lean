...
structure RewrittenAt
  ...
  /-- A returned value that already exists in the source context dominates the replacement point.
  This is the source-context `ReturnValuesDominate` fact. Fresh results need no unconditional scope
  property: target dominance constrains them at each actual use, while unused results need not
  dominate an arbitrary program point. -/
  returnedValuesDominate : ∀ v ∈ newValues, v.InBounds ctx.raw →
...
the image `σ r` being a target block argument makes it the same pre-existing source block argument.
`returnedValuesDominate` puts it in scope before `op`, and dominance transport puts it in scope after
the unchanged target prefix. Thus `bl` dominates `block`, while `hdom` forces `block` to dominate
`bl`; antisymmetry gives `bl = block`, a contradiction. Discharges the `hImageNotArg` hypothesis of
`setArgumentValues?_isRefinedByAt`. -/
theorem mappingImageNotArg
  ...
  · -- `val` is a result `r` of `op`, which lives in `block`.
    ...
    · -- `bl ≠ block`: cross-block antisymmetry rules out `σ r ∈ bl.args`.
      ...
      have hσMemSrc : (h.σ ⟨val, valIn⟩).val ∈ bl.getArguments! ctx.raw := by
        rw [← h.getArguments!_eq blIn]
        exact hσMem
      have hσInSrc : (h.σ ⟨val, valIn⟩).val.InBounds ctx.raw := by
        grind [BlockPtr.getArguments!.mem_iff_exists_index]
      have hσDomBefore := h.returnedValuesDominate _ hImgMem hσInSrc
      have hσDom := h.value_dominatesIp_before_transport _ hσInSrc hσDomBefore
      have hσNotResult : (h.σ ⟨val, valIn⟩).val ∉ op.getResults! ctx.raw := by
        obtain ⟨i, hi, heq⟩ := BlockPtr.getArguments!.mem_iff_exists_index.mp hσMemSrc
        rw [← heq]
        exact h.blockArg_notMem_getResults
      rw [h.mappingFixesNonResults _ hσInSrc hσNotResult] at hσDom
      have hops : ∀ o ∈ pre.toList,
          o ∈ block.operationList newCtx.raw newCtx.wellFormed blockIn' := by
        intro o ho
        rw [h.tgtList]
        have : o ∈ pre := by simpa using ho
        exact Array.mem_append.mpr (Or.inl (Array.mem_append.mpr (Or.inl this)))
...
theorem PatternRewriter.eraseOp!_eq_eraseOp {b : PatternRewriter OpCode} {op : OperationPtr}
  ...
  simp only [PatternRewriter.eraseOp!, PatternRewriter.eraseOp]
  grind
...
def OperationPtr.SameIntrinsic (o : OperationPtr) (c c' : IRContext OpCode) : Prop :=
  ...
  (∀ (ot : OpCode), o.getProperties! c' ot = o.getProperties! c ot) ∧
...
theorem WfIRContext.WithCreatedOps.sameIntrinsic {ctx₁ ctx₂ : WfIRContext OpCode}
  ...
  | Nil => rfl
  | CreatedOp ctx₁ ctx₂ ctx₃ hwco hex ih =>
    grind [OperationPtr.SameIntrinsic]
...
theorem OperationPtr.getParentOp!_eq_of {c c' : IRContext OpCode} {o m : OperationPtr}
  ...
  rw [OperationPtr.getParentOp!_eq_some_iff] at h ⊢
  obtain ⟨b, r, hb, hr, h⟩ := h
  have hbIn : b.InBounds c := by grind [OperationPtr.parent!_inBounds]
  have hrIn : r.InBounds c := by grind [BlockPtr.parent!_inBounds]
  refine ⟨b, r, ?_, ?_, ?_⟩
  · simpa only [hop] using hb
  · simpa only [hblk b] using hr
  · simpa only [hrgn r hrIn (by rw [h]; exact Option.some_ne_none m)] using h
...
theorem RewrittenAt.of_fromLocalRewrite
  ...
  obtain ⟨-, hReturnCtxChanges, hReturnOps, hReturnOpsNodup, hReturnValues, hReturnValuesInBounds,
    ...
    hRewritePreservesDom, hRewritePreservesVerified,
  ...
  rw [hpat] at hdriver
  simp only [bind_pure_comp] at hdriver
  ...
  rw [Array.forIn_pure_yield_eq_foldl] at hfold1 hfold2
  -- `heraseRaw : s₂.eraseOp! op = rewriter'`. The two folds as `List.foldlM`s.
  have hfold1L :
      List.foldlM (fun b a => some (b.insertOp! a (InsertPoint.before op)))
        ⟨newCtxPat, true, rewriter.worklist⟩ newOps.toList = some s₁ := by
    change List.foldlM (fun b a => pure (b.insertOp! a (InsertPoint.before op)))
      ⟨newCtxPat, true, rewriter.worklist⟩ newOps.toList = pure s₁
    rw [List.foldlM_pure, Array.foldl_toList]
    exact hfold1
  have hfold2L :
      List.foldlM
        (fun b a => some (b.replaceValue! (ValuePtr.opResult (op.getResult a.2)) a.1))
        s₁ newValues.zipIdx.toList = some s₂ := by
    change List.foldlM
      (fun b a => pure (b.replaceValue! (ValuePtr.opResult (op.getResult a.2)) a.1))
      s₁ newValues.zipIdx.toList = pure s₂
    rw [List.foldlM_pure, Array.foldl_toList]
    exact hfold2
  ...
  have hResNe : ∀ a ∈ newValues.zipIdx.toList, (ValuePtr.opResult (op.getResult a.2)) ≠ a.1 := by
    intro a ha
    intro heq
    apply hReturnValuesNotOwnResults rewriter.ctx op newCtxPat newOps newValues hpat a.1
      (hzmem a ha)
    rw [← heq]
    apply OperationPtr.getResults!.mem_getResult
    rw [hNumRes]
    exact hzlt a ha
  have hResNoown : ∀ a ∈ newValues.zipIdx.toList, ∀ a' ∈ newValues.zipIdx.toList,
      a.1 ≠ (ValuePtr.opResult (op.getResult a'.2)) := by
    intro a ha a' ha' heq
    apply hReturnValuesNotOwnResults rewriter.ctx op newCtxPat newOps newValues hpat a.1
      (hzmem a ha)
    rw [heq]
    ...
    have hframe : op.getNumResults! s₂.ctx.raw = newValues.size := by
      have h1 : op.getNumResults! s₁.ctx.raw = op.getNumResults! newCtxPat.raw :=
        (PatternRewriter.insertOp!_before_fold_invariant
          ...
          (fun _ _ _ _ _ hins => by
            rw [(PatternRewriter.insertOp_sameIntrinsic (o := op) hins).2.2.1])
          hopNewCtxPat (by simp [hparInit]) hNewOpsBnd hNewOpsDet hNewOpsNodup hopNotNewOpsL hfold1L).mpr rfl
      have h2 : op.getNumResults! s₂.ctx.raw = op.getNumResults! s₁.ctx.raw :=
        (PatternRewriter.replaceValue!_fold_invariant
          ...
          (fun _ _ _ _ _ _ => by rw [(PatternRewriter.replaceValue_sameIntrinsic (o := op)).2.2.1])
          hfold2L hResNe hResOldIn hResNewIn).mpr rfl
      rw [h2, h1, hNumRes]
  ...
  have hR : RewrittenAt rewriter.ctx op newOps newValues rewriter'.ctx opInBounds
    ...
    newValuesInBounds := by
      intro v hv
      have hvIn :=
        hReturnValuesInBounds rewriter.ctx op newCtxPat newOps newValues hpat v hv
      apply hSurviveVal v hvIn
      intro orp hvorp heq
      apply hReturnValuesNotOwnResults rewriter.ctx op newCtxPat newOps newValues hpat v hv
      obtain ⟨o', i'⟩ := orp
      change o' = op at heq
      subst o'
      rw [hvorp]
      apply OperationPtr.getResults!.mem_getResult
      rw [hvorp, ValuePtr.inBounds_opResult, OpResultPtr.inBounds_def] at hvIn
    ...
    frame := by
      ...
      have hNoAlias : ∀ x ∈ newValues, ∀ m, x ≠ (op.getResult m : ValuePtr) := by
        intro x hx m heq
        have hxIn :=
          hReturnValuesInBounds rewriter.ctx op newCtxPat newOps newValues hpat x hx
        apply hReturnValuesNotOwnResults rewriter.ctx op newCtxPat newOps newValues hpat x hx
        rw [heq]
        apply OperationPtr.getResults!.mem_getResult
        rw [heq, ValuePtr.inBounds_opResult, OpResultPtr.inBounds_def] at hxIn
        grind [OperationPtr.getResult]
    ...
    -- The only unconditional dominance fact needed for returned values is the existing-value case,
    -- which is exactly the source-context `ReturnValuesDominate` pattern obligation.
    returnedValuesDominate :=
      hReturnValuesDominate rewriter.ctx op newCtxPat newOps newValues hpat
...
(`LocalRewritePattern.Sound.returnsCtxNoChanges`), so driver soundness on that branch reduces to
...
matched, in-bounds `op` on a `Sound` pattern over a verified, dominance-wellformed context refines
every module.
...
that is (a) established for `fromLocalRewrite` of a `Sound` local pattern by
...
/-- The driver applied to a `Sound` local rewrite is a sound `RewritePattern`. The three components
are precisely `Sound.rewritePreservesDom`, `Sound.rewritePreservesVerified` and the headline
...
theorem RewritePattern.Sound.fromLocalRewrite {pattern : LocalRewritePattern OpCode}
    (hValid : pattern.Sound) : (RewritePattern.fromLocalRewrite pattern).Sound := fun
...
-/
private def GreedyRel (s s' : Option (PatternRewriter OpCode) × PatternRewriter OpCode) : Prop :=
...
/-- Greedy composition of the drivers of `Sound` local rewrite patterns is a sound `RewritePattern`. -/
theorem RewritePattern.Sound.greedy_fromLocalRewrite {patterns : Array (RewritePattern OpCode)}
    (hValid : ∀ p ∈ patterns, ∃ q : LocalRewritePattern OpCode,
      q.Sound ∧ p = RewritePattern.fromLocalRewrite q) :
    (RewritePattern.GreedyRewritePattern patterns).Sound := by
...
`GreedyRewritePattern` over the drivers of `Sound` local rewrite patterns, on an in-bounds operation
...
the greedy composition of the drivers of `Sound` local rewrite patterns returns a
