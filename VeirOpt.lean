...
structure VeirOptArgs where
  ...
  /-- Whether to print operations using their custom (non-generic) assembly format. -/
  pretty : Bool
...
def parseArgs (args : List String) : Except String VeirOptArgs := do
  ...
  if positional.length == 0 then -- read from stdin
    return { filename := none, passes := pipeline, allowUnregisteredDialect, disableVerifiers, pretty }
  ...
  return { filename := some filename, passes := pipeline, allowUnregisteredDialect, disableVerifiers, pretty }
...
def main (args : List String) : IO Unit := do
  ...
  | .error errMsg =>
    ...
    IO.eprintln "Usage: veir-opt <filename> [-p=\"pass1,pass2,...\"]... [--allow-unregistered-dialect] [--disable-verifiers] [--pretty]"
    ...
    IO.Process.exit 1
  | .ok { filename, passes, allowUnregisteredDialect, disableVerifiers, pretty } =>
    ...
    | .ok (ctx, op) =>
      ...
      | .error errMsg =>
        ...
        IO.Process.exit 1
      | .ok finalCtx =>
        Veir.Printer.printOperation finalCtx.raw op pretty
