...
| Felt semantic model | ✅ Supported | **Phase E.5 (2026-05-19)** upgraded from provisional `abbrev Felt := Int` to `abbrev Felt (p : Nat) := ZMod p` via Mathlib. All four verified Felt rewrites (`right_identity_zero_add`, `constant_fold_add`, `self_subtraction_to_zero`, `assoc_const_fold_add`) re-proved with the modulus universally quantified — the IR doesn't pin a specific field, so passes must be sound for arbitrary `p`. Mathlib's `CommRing (ZMod p)` instances discharge each identity directly (`simp` or `push_cast; ring`). Axiom footprint: only `propext` and `Quot.sound` (standard Lean). Toolchain pinned to `v4.30.0-rc2` to match Mathlib master. |
