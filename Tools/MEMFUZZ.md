...
survive. Today that is `alloca`, the heap family, `getelementptr`, loads and stores of
both integers and pointers, `memcpy`, and the pointer/integer casts, with
out-of-bounds offsets, use after free, misalignment and null dereferences drawn
at tunable rates.

Two options stay off because the reference cannot answer for them. `alive-exec`
