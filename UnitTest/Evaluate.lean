...
/-- Memory operations must not be evaluated: a load would read the dummy
    memory `foldEvaluate` supplies, and a store would write to it and throw
    the result away. Non-volatile loads are the trap here, since
    `hasSideEffects` classifies them as pure. -/
private def testMemoryOpsAreNotEvaluable : String := Id.run do
  let memoryOps : Array OpCode :=
    #[.llvm .load, .llvm .store,
      .riscv .ld, .riscv .lw, .riscv .lwu, .riscv .lh, .riscv .lhu,
      .riscv .lb, .riscv .lbu,
      .riscv .sd, .riscv .sw, .riscv .sh, .riscv .sb]
  for op in memoryOps do
    if op.isFoldEvaluable then
      return s!"a memory operation was classified as evaluable"
  -- Pure arithmetic in the same dialects stays evaluable.
  for op in #[OpCode.llvm .add, .riscv .add, .arith .addi] do
    if ¬ op.isFoldEvaluable then
      return "a pure arithmetic operation was classified as not evaluable"
  return "ok"
...
-/
#guard_msgs in
#eval! testMemoryOpsAreNotEvaluable

/-- Control flow is not evaluable: `foldEvaluate` has no block arguments to
    supply and no way to report a branch target. -/
private def testControlFlowIsNotEvaluable : String := Id.run do
  for op in #[OpCode.cf .br, .cf .cond_br, .riscv_cf .beq, .func .return] do
    if op.isFoldEvaluable then
      return "a control-flow operation was classified as evaluable"
  return "ok"
...
-/
#guard_msgs in
#eval! testControlFlowIsNotEvaluable
