...
example : diverge 0 = .ub := by
  apply diverge.fixpoint_induct 0 (motive := fun f => f = .ub)
  · exact Interp.admissible_of_ub _ rfl
  · intro f ih
    exact ih

/-- `ub` is below every outcome. -/
example (x : Interp Nat) : Lean.Order.PartialOrder.rel .ub x :=
  Interp.rel_iff.mpr (.inl rfl)

/-- Nothing else is. -/
example : ¬ Lean.Order.PartialOrder.rel (.fail : Interp Nat) .ub := by
  intro h
