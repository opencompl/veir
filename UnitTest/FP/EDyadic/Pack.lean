...
/-! ## Round-trip tests against Lean's native `Float` (IEEE-754 double)

For every double `f`, packing `f.toEDyadic` should recover the original
packed bit pattern. This exercises the normal range, ±0, ±∞, and the
subnormal range. -/
...
NaN packs to a canonical NaN representation (biased exponent all-ones,
trailing significand `1`). The `toEDyadic` direction collapses every NaN
bit pattern to `EDyadic.nan`, so we check the canonical form rather than
strict round-tripping. -/
...
Round-tripping at the boundary of the subnormal range. -/
...
Exercise the same round-trip on a non-double format. The `(e, s) = (5, 10)`
half-precision-like format keeps the test values small enough to be
checked by hand. -/
