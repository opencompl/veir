...
private theorem doubleAdd_valid : doubleAdd.Valid := by
  unfold doubleAdd doubleAddCreation matchDoubleAdd
  provePuddleValid
  intro type _ lhs _ rhs _ innerProperties innerValue hInnerNsw hInnerNuw hInner
    rootProperties rootValue hRootNsw hRootNuw hRoot
  rcases innerProperties with ⟨⟨innerNsw, innerNuw⟩⟩
  rcases rootProperties with ⟨⟨rootNsw, rootNuw⟩⟩
  change innerNsw = false at hInnerNsw
  change innerNuw = false at hInnerNuw
  change rootNsw = false at hRootNsw
  change rootNuw = false at hRootNuw
  subst innerNsw
  subst innerNuw
  subst rootNsw
  subst rootNuw
  exact ⟨innerValue, hInner, rootValue, hRoot,
    RuntimeValue.isRefinedBy_refl rootValue⟩
...
#guard matcherAccepts floatMatcher FloatType.f32
...
#guard !(matcherAccepts pointerMatcher FloatType.f64)
