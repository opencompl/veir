...
private def createOrFoldTestModule : String :=
  "\"builtin.module\"() ({
...
/-- How many operations of the given opcode the context holds. -/
private def countOps (ctx : IRContext OpCode) (opType : OpCode) : Nat :=
  ctx.operations.keys.countP (·.getOpType! ctx = opType)

private def testCreateOrFold : String := Id.run do
  match parseTopLevelOp createOrFoldTestModule with
  | .error e => return s!"parse error: {e}"
  | .ok (_, parserState) =>
    let ctx := parserState.ctx
    let i32 : TypeAttr := IntegerType.mk 32
    let some ⟨add, addInBounds⟩ := findOp ctx.raw (.arith .addi)
      | return "missing arith.addi"
    let some ⟨ret, _⟩ := findOp ctx.raw (.func .return)
      | return "missing func.return"
    let some entryBlock := (ret.get! ctx.raw).parent
      | return "func.return has no parent block"
    let constants := add.getOperands ctx.raw addInBounds
    let some arg := (entryBlock.getArguments! ctx.raw)[0]?
      | return "missing block argument"
    let rewriter : PatternRewriter OpCode :=
      { ctx, hasDoneAction := false, worklist := .empty }

    -- Both operands constant: no `arith.addi` is created, and the result is a
    -- freshly materialized 15.
    let some (folded, results) := rewriter.createOrFold! (.arith .addi) #[i32] constants
      #[] #[] default (.before ret)
      | return "createOrFold! failed on constant operands"
    if countOps folded.ctx.raw (.arith .addi) ≠ 1 then
      return "createOrFold! created an arith.addi it should have folded away"
    match results[0]?.bind (ValuePtr.constantValue · folded.ctx.raw) with
    | some (.int 32 (.val 15)) => pure ()
    | _ => return "createOrFold! did not fold to the constant 15"

    -- A non-constant operand defeats the fold, so the operation is built.
    let some (created, results) := rewriter.createOrFold! (.arith .addi) #[i32] #[arg, constants[0]!]
      #[] #[] default (.before ret)
      | return "createOrFold! failed on a non-constant operand"
    if countOps created.ctx.raw (.arith .addi) ≠ 2 then
      return "createOrFold! did not create the unfoldable arith.addi"
    let some (result : ValuePtr) := results[0]?
      | return "createOrFold! returned no result"
    let .opResult res := result
      | return "createOrFold! did not return an operation result"
    if res.op.getOpType! created.ctx.raw ≠ .arith .addi then
      return "createOrFold! returned a result of the wrong operation"
