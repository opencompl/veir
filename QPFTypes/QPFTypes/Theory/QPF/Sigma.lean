...
data types like `Nat` or over `Type.{u-1}` -/
...
instance : QPF (Sigma F) where
  ...
  abs_repr := by
    rintro α ⟨x, f⟩;
    have : ⟨(repr f).fst, (repr f).snd⟩ = repr f := by rfl
    simp only [Sigma.abs, Sigma.repr, this, abs_repr]
  abs_map := by
    rintro α β f ⟨x, g⟩;
...
protected def abs ⦃α⦄ : Pi.P F α → Pi F α
  | ⟨a, f⟩ => fun x => QPF.abs ⟨a x, fun i y => f i ⟨_, y⟩⟩

/-- representation function for dependent products -/
protected def repr ⦃α⦄ : Pi F α → Pi.P F α
  | f => ⟨fun a => (QPF.repr (f a)).1, fun _i a => (QPF.repr (f _)).2 _ a.2⟩

set_option backward.isDefEq.respectTransparency false in
instance : QPF (Pi F) where
  P := Pi.P F
  abs := @Pi.abs _ _ F _
